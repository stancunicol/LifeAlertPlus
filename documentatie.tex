...
\begin{tikzpicture}[
    every node/.style={font=\small},
    input/.style={rectangle, rounded corners, draw, minimum width=4.0cm, minimum height=1.0cm, align=center, fill=blue!8},
    proc/.style={rectangle, rounded corners, draw, minimum width=4.0cm, minimum height=1.0cm, align=center, fill=orange!15},
    rules/.style={rectangle, rounded corners, draw, minimum width=4.0cm, minimum height=1.0cm, align=center, fill=yellow!20},
    fuz/.style={rectangle, rounded corners, draw, minimum width=8.6cm, minimum height=1.1cm, align=center, fill=red!10},
    out/.style={rectangle, rounded corners, draw, minimum width=4.0cm, minimum height=1.0cm, align=center, fill=green!15},
    arrow/.style={-{Latex[length=2mm]}, thick},
    join/.style={draw, thick}]

  \node[input] (sens) at (-2.3,0) {Valori senzori\\\scriptsize puls, \spo, T, IMU};
  \node[input] (prag) at (2.3,0) {Praguri + afecțiuni\\\scriptsize personalizate pe pacient};

  \node[proc] (rf) at (-2.3,-3.0) {Random Forest\\\scriptsize predicție + probabilități};
  \node[rules] (rg) at (2.3,-3.0) {Reguli medicale\\\scriptsize praguri absolute};

  \node[fuz] (fuziune) at (0,-5.2) {Strat de fuziune și severitate\\\scriptsize combinație predicție + reguli + modificatori};

  \node[out] (stare) at (-2.3,-7.4) {Stare finală\\\scriptsize NORMAL / ALERT / CRITICAL};
  \node[out] (score) at (2.3,-7.4) {Health score\\\scriptsize 0--100 + explicație};

  % Fan-out from inputs to both processors via a junction at y=-1.5
  \draw[join] (sens.south) -- (-2.3,-1.5);
  \draw[join] (prag.south) -- (2.3,-1.5);
  \draw[join] (-2.3,-1.5) -- (2.3,-1.5);
  \draw[arrow] (-2.3,-1.5) -- (rf.north);
  \draw[arrow] (2.3,-1.5) -- (rg.north);

  % Fan-in from processors to the fusion box via a junction at y=-4.0
  \draw[join] (rf.south) -- (-2.3,-4.0);
  \draw[join] (rg.south) -- (2.3,-4.0);
  \draw[join] (-2.3,-4.0) -- (2.3,-4.0);
  \draw[arrow] (0,-4.0) -- (fuziune.north);

  % Fan-out from fusion to two outputs via a junction at y=-6.4
  \draw[join] (fuziune.south) -- (0,-6.4);
  \draw[join] (-2.3,-6.4) -- (2.3,-6.4);
  \draw[arrow] (-2.3,-6.4) -- (stare.north);
  \draw[arrow] (2.3,-6.4) -- (score.north);
\end{tikzpicture}
\caption{Mecanismul hibrid de decizie: modelul Random Forest cooperează cu setul de reguli medicale pentru a produce starea finală și health score-ul.}
\label{fig:hibrid-ai}
\end{figure}
...
\figplaceholder{[Spațiu pentru figură: captură de ecran cu tabloul de bord al aplicației web, care prezintă starea curentă a persoanei monitorizate, parametrii vitali și health score-ul.]}
