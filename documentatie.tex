...
CI/CD & Continuous Integration / Continuous Delivery --- integrare și livrare continuă\\
...
HTTPS & HyperText Transfer Protocol Secure --- HTTP securizat\\
...
Provocarea de a integra mai multe tehnologii moderne --- senzori IoT, comunicație wireless, backend scalabil, aplicație web și mecanisme de notificare prin SMS, email și notificări push --- a reprezentat un factor motivant suplimentar. Complexitatea tehnică a proiectului oferă oportunitatea aplicării practice a cunoștințelor acumulate pe parcursul studiilor, într-un scenariu cu relevanță reală.
...
Dincolo de aspectul tehnic, lucrarea urmărește să arate că o astfel de soluție poate contribui la prevenirea complicațiilor medicale prin detectarea timpurie a anomaliilor și notificarea automată a persoanelor responsabile --- aparținători, medici sau servicii de urgență. În situațiile critice, intervenția rapidă este sprijinită de localizarea GPS a pacientului și de un buton de panică prin care acesta poate solicita direct ajutor.
...
Capitolul IV descrie proiectarea sistemului --- arhitectura generală, fluxul de date, structura bazei de date și logica mecanismului health score.
...
\subsubsection{Senzorul MAX30100 --- puls și saturație de oxigen}
...
\subsubsection{Senzorul MPU6050 --- mișcare și accelerație}
...
% maparea pinilor GPIO ai ESP32-C3 către senzori, multiplexor, OLED și butoane.

% ---------- 2.3 (NOU) ----------
\subsection{Prelucrarea datelor și inteligență artificială}

Datele brute furnizate de senzorii biomedicali nu pot fi utilizate direct pentru a decide dacă starea pacientului este una normală sau una care necesită intervenție. Valorile individuale --- o frecvență cardiacă de 105 bpm, o temperatură de 37,8\textdegree C --- sunt ambigue atunci când sunt interpretate izolat: ele pot indica fie o stare patologică, fie o reacție fiziologică firească la efort, stres sau temperatura ambientală. Din acest motiv, transformarea datelor în informație utilă din punct de vedere clinic presupune o etapă de prelucrare și o etapă de interpretare, în care intervine inteligența artificială.
...
Învățarea automată (machine learning) este subdomeniul inteligenței artificiale care studiază algoritmii capabili să își îmbunătățească performanța pe baza datelor, fără a fi programați explicit pentru fiecare situație posibilă. În cazul de față, evaluarea stării pacientului a fost formulată ca o problemă de \textbf{clasificare supervizată}: pornind de la un set de parametri vitali, modelul trebuie să atribuie pacientului una dintre trei clase --- \texttt{NORMAL}, \texttt{ALERT} sau \texttt{CRITICAL}. Clasificarea supervizată presupune existența unui set de date de antrenare în care fiecare exemplu este însoțit de eticheta corectă, modelul deducând din aceste exemple o funcție de decizie care poate fi aplicată ulterior unor date noi.
...
% >>> FIGURĂ SUGERATĂ: schema sistemului hibrid de decizie --- model Random Forest
% combinat cu reguli medicale, conducând la starea finală și la health score.
...
Combinarea celor trei canale asigură redundanță: dacă un canal eșuează sau nu este observat, un altul preia rolul de informare, crescând probabilitatea ca aparținătorul să fie alertat la timp.
...
Persistența datelor este asigurată de o bază de date relațională \textbf{SQLite}, un sistem de baze de date încorporat, care stochează întreaga bază într-un singur fișier și nu necesită un server dedicat. Accesul la baza de date se realizează prin intermediul unui ORM (Object-Relational Mapper) --- \textbf{Entity Framework Core} --- care mapează automat clasele din cod pe tabelele bazei de date și permite formularea interogărilor direct în limbajul C\#.
...
Interfața web a fost dezvoltată folosind \textbf{Blazor WebAssembly}, un framework care permite scrierea aplicațiilor web interactive în limbajul C\#, fără a fi necesar JavaScript pentru logica aplicației. Codul C\# este compilat în \textbf{WebAssembly}, un format binar de instrucțiuni care rulează direct în browser, la o viteză apropiată de cea nativă. Aplicația este o \textbf{SPA (Single-Page Application)} --- o aplicație web care se încarcă o singură dată și care actualizează dinamic conținutul, fără reîncărcarea completă a paginii.
...
Pentru asigurarea consistenței între mediul de dezvoltare și cel de producție, componentele aplicației sunt împachetate folosind \textbf{Docker}, o tehnologie de containerizare care încapsulează o aplicație împreună cu toate dependențele sale într-o imagine portabilă. Cele trei componente principale ale sistemului --- microserviciul AI, backend-ul și frontend-ul --- sunt orchestrate prin \textbf{Docker Compose}, un instrument care permite definirea și pornirea coordonată a mai multor containere. Servirea fișierelor statice ale interfeței web este realizată printr-un server \textbf{nginx}.
...
Philips Lifeline și Medical Guardian reprezintă o categorie distinctă de dispozitive, anume sistemele de alertă personală medicală (PERS --- Personal Emergency Response Systems), dedicate exclusiv persoanelor vârstnice sau cu mobilitate redusă. Acestea iau forma unor butoane de panică purtate ca pandantiv sau brățară, conectate la o centrală care permite contactarea unui operator uman sau a serviciilor de urgență.

Avantajul principal al acestor sisteme este simplitatea extremă din perspectiva utilizatorului --- un singur buton fizic. Dezavantajul major este că nu realizează monitorizare continuă a parametrilor vitali. Sistemul reacționează exclusiv la inițiativa pacientului, ceea ce îl face ineficient în scenarii în care pacientul este incapabil să apese butonul --- pierderea conștienței, cădere cu imobilizare, degradare lentă a stării de sănătate.
...
Dispozitivele comerciale de tip smartwatch alertează utilizatorul însuși, nu un aparținător desemnat. Scenariul critic --- pacient incapabil să reacționeze, aparținător neinformat --- nu este acoperit adecvat de niciuna dintre soluțiile analizate. Sistemele PERS depind exclusiv de inițiativa pacientului, iar smartwatch-urile nu oferă un mecanism fiabil de escaladare a alertelor către persoane terțe prin canale multiple (SMS, email, push).
...
Atât Apple Watch, cât și sistemele PERS implică costuri recurente --- abonamente cloud, taxe lunare pentru servicii de monitorizare umană --- care pot fi prohibitive pe termen lung, în special pentru persoane cu venituri fixe reduse, categorie frecvent suprapusă cu cea a vârstnicilor.
...
\item \textbf{Infrastructure} --- conține implementările concrete: contextul bazei de date și depozitele realizate cu Entity Framework Core, precum și integrările cu servicii externe. Depinde de Application și Domain.
...
Componentele sistemului comunică prin trei mecanisme distincte. Dispozitivul IoT și aplicația web comunică cu backend-ul prin \textbf{API-ul REST}, peste HTTPS. Backend-ul comunică cu microserviciul AI tot prin HTTP. În fine, pentru actualizarea instantanee a interfeței și pentru livrarea notificărilor push, backend-ul comunică cu aplicația web printr-o conexiune permanentă \textbf{WebSocket}, gestionată de biblioteca SignalR. Cele trei componente principale --- microserviciul AI, backend-ul și frontend-ul --- sunt împachetate în containere Docker și orchestrate prin Docker Compose, ceea ce asigură consistența între mediul de dezvoltare și cel de producție. Ansamblul componentelor și al legăturilor dintre ele este prezentat în figura~\ref{fig:arhitectura}.

\begin{figure}[H]
\centering
% Înlocuiește linia de mai jos cu imaginea reală, de exemplu:
% \includegraphics[width=0.9\textwidth]{figuri/arhitectura.png}
\figplaceholder{[Spațiu pentru figură --- diagrama de arhitectură a sistemului: dispozitivul IoT (ESP32 + senzori), nivelul de rețea (WiFi/BLE), backend-ul, microserviciul AI, baza de date și aplicația web, cu protocoalele de comunicație dintre ele (HTTPS/REST, HTTP, WebSocket).]}
\caption{Arhitectura generală a sistemului LifeAlertPlus.}
\label{fig:arhitectura}
\end{figure}

\subsubsection{Modelul de deployment}

Cele trei componente software ale sistemului --- microserviciul AI, backend-ul și aplicația web --- sunt împachetate fiecare într-o imagine Docker proprie și pornite coordonat. Această izolare a fost o decizie de proiectare deliberată: fiecare componentă poate fi actualizată, repornită sau scalată independent de celelalte, iar mediul de execuție rămâne identic în dezvoltare și în producție. Datele persistente --- baza de date SQLite --- sunt păstrate într-un volum separat de containere, astfel încât informația să nu se piardă la reconstruirea sau actualizarea acestora.
...
Structura claselor principale ale aplicației --- entități, servicii și controlere --- este prezentată în diagrama de clase din figura~\ref{fig:clase}.

\begin{figure}[H]
\centering
% Exportă diagrama din LifeAlertPlusClassDiagram.vpp ca imagine și inserează-o:
% \includegraphics[width=\textwidth]{figuri/diagrama-clase.png}
\figplaceholder{[Spațiu pentru figură --- diagrama de clase a aplicației, exportată din fișierul LifeAlertPlusClassDiagram.vpp: clasele de domeniu, serviciile și controlerele, cu relațiile dintre ele.]}
\caption{Diagrama de clase a aplicației LifeAlertPlus.}
\label{fig:clase}
\end{figure}
...
\centering
% Înlocuiește linia de mai jos cu imaginea reală, de exemplu:
% \includegraphics[width=0.9\textwidth]{figuri/model-date.png}
\figplaceholder{[Spațiu pentru figură --- diagrama entitate-relație: cele 11 entități, atributele lor și relațiile dintre ele, cu evidențierea relației mulți-la-mulți User--Monitored prin tabela UserMonitored.]}
...
\item \textbf{Imobilitatea} --- o accelerație foarte redusă, care poate indica o cădere urmată de imobilizare --- adaugă o contribuție suplimentară.
...
\begin{figure}[H]
\centering
\begin{tikzpicture}[node distance=2.0cm and 2.8cm,
    every node/.style={font=\small},
    state/.style={rectangle, rounded corners, draw, minimum width=2.8cm, minimum height=1.0cm, align=center, fill=blue!5},
    edge/.style={-{Latex[length=2mm]}, thick}]

  \node[state] (idle) {IDLE\\\scriptsize{așteaptă free-fall}};
  \node[state, right=of idle] (ff) {IN\_FREEFALL\\\scriptsize{|a| < 0,5\,g}};
  \node[state, right=of ff] (st) {MONITORING\_\\STILLNESS\\\scriptsize{|a-1g| < 0,25\,g}};
  \node[state, below=1.6cm of st, fill=green!10] (conf) {FALL\_CONFIRMED\\\scriptsize{raportat la POST}};

  \draw[edge] (idle) -- node[above, font=\scriptsize]{|a| < 0,5g} (ff);
  \draw[edge] (ff) -- node[above, font=\scriptsize]{impact $\geq$ 2,5g} node[below, font=\scriptsize]{după $\geq$ 80\,ms FF} (st);
  \draw[edge] (st) -- node[right, font=\scriptsize]{stillness 1,5\,s} (conf);
  \draw[edge, bend left=22] (ff) to node[below, font=\scriptsize]{abort (mișcare / timeout)} (idle);
  \draw[edge, bend left=35] (conf.west) to node[below, font=\scriptsize]{reset} (idle.south);
\end{tikzpicture}
...
\begin{figure}[H]
\centering
\figplaceholder{[Spaţiu pentru figură --- schema electrică a dispozitivului: ESP32-C3 Mini, multiplexorul TCA9548A, senzorii MAX30100/MPU6050/DS18B20/NEO-6M, ecranul OLED, butoanele şi LED-urile, cu pinii GPIO corespunzători şi rezistoarele de pull-up evidenţiate.]}
...
Accesul la pagini este controlat în funcție de rolul utilizatorului: paginile de administrare sunt disponibile doar administratorilor, iar fiecare aparținător sau medic vede exclusiv persoanele monitorizate la care are drept de acces. Tentativele de acces neautorizat sunt redirecționate către pagina de autentificare. Întrucât aplicația se adresează inclusiv unui public mai puțin familiarizat cu tehnologia, interfața include opțiuni de accesibilitate --- selectarea limbii (română sau engleză), comutarea limbii fiind reflectată în timp real pe toate paginile --- precum și un asistent conversațional integrat, care răspunde la întrebări frecvente despre utilizarea aplicației.

\subsubsection{Organizarea interfeței și gestionarea stării}

Interfața este construită din componente Blazor reutilizabile --- antetul, widget-urile și notificările de tip \textit{toast} --- combinate în pagini. Starea partajată între pagini, precum datele utilizatorului autentificat sau persoana monitorizată selectată, este menținută de servicii dedicate, înregistrate în containerul de injecție de dependențe al aplicației client. Astfel, informațiile încărcate o singură dată sunt disponibile în întreaga aplicație, fără a fi nevoie de cereri repetate către server la fiecare schimbare de pagină.

\subsubsection{Vizualizarea datelor și funcționalitatea PWA}

Reprezentarea grafică a istoricului parametrilor vitali este realizată prin interoperabilitate cu JavaScript, folosind o bibliotecă de grafice; tot prin interoperabilitate sunt integrate harta pentru localizarea pacientului și exportul rapoartelor în format PDF. Aplicația este configurată ca \textbf{PWA}: include un service worker, un fișier manifest care permite instalarea pe dispozitiv și o pagină dedicată modului offline.

\begin{figure}[H]
\centering
% \includegraphics[width=\textwidth]{figuri/dashboard.png}
\figplaceholder{[Spațiu pentru figură --- captură de ecran cu tabloul de bord al aplicației web: starea curentă a persoanei monitorizate, parametrii vitali și health score-ul.]}
\caption{Tabloul de bord al aplicației web.}
\label{fig:dashboard}
\end{figure}

% >>> FIGURI SUGERATE (capturi de ecran din aplicație) pentru această secțiune:
%  - pagina de autentificare / înregistrare;
%  - lista persoanelor monitorizate;
%  - pagina de istoric cu graficele parametrilor vitali;
%  - pagina de setări (configurarea pragurilor de alertă personalizate);
%  - vizualizarea pe hartă a poziției pacientului.

\subsection{Sistemul de notificări}

Sistemul de notificări implementează alertarea multi-canal descrisă conceptual în subcapitolul 2.4. Logica de declanșare este coordonată de serviciul de monitorizare a alertelor, care decide când și prin ce canale trebuie transmisă o notificare, ținând cont de preferințele fiecărui utilizator.

\subsubsection{Temporizarea alertelor}

Pentru a evita alertele false, serviciul de monitorizare aplică mai multe temporizări. O stare anormală trebuie să persiste o durată minimă --- pragul de persistență, de aproximativ două minute pentru stările critice --- înainte de a genera o notificare, eliminând valorile tranzitorii. La declanșare, alerta este transmisă simultan prin toate canalele activate de utilizator (push, email și SMS), fiecare alertă fiind însoțită de SMS dacă acest canal este selectat; SMS-ul nu este întârziat suplimentar față de celelalte canale. Atâta vreme cât starea critică persistă, notificarea este retrimisă la fiecare două minute; pentru alertele non-critice, un interval de inhibare (\textit{cooldown}) împiedică retransmiterea aceleiași alerte pentru aceeași persoană, eliminând riscul inundării aparținătorului cu mesaje. Pentru aplicarea acestor reguli, serviciul menține în memorie, pentru fiecare persoană monitorizată, un buffer cu măsurătorile din ultimele minute.
...
Testarea sistemului a fost realizată pe mai multe planuri. \textbf{Testarea funcțională} a vizat verificarea fiecărui flux descris în subcapitolul 4.4 --- înregistrarea utilizatorilor, asocierea unei persoane monitorizate, recepția măsurătorilor, declanșarea alertelor și livrarea notificărilor.

Pentru testarea fluxurilor care depind de dispozitivul fizic a fost utilizat un \textbf{mod de simulare}, integrat în aplicație, care permite generarea de măsurători cu valori controlate, fără a fi necesar hardware-ul real. Acest mecanism a permis reproducerea sistematică a scenariilor de risc --- valori critice de \spo, tahicardie, febră, imobilitate --- și verificarea reacției corecte a sistemului în fiecare caz.
...
Latența reprezintă intervalul de timp scurs între achiziția unei măsurători pe dispozitiv și afișarea sau alertarea acesteia (obiectivul O5). Au fost măsurate două intervale: timpul \textit{dispozitiv -- afișare}, de la citirea senzorilor până la actualizarea tabloului de bord, și timpul \textit{dispozitiv -- alertă}, de la achiziție până la livrarea notificării push.

Latența totală este suma mai multor componente: intervalul de transmitere configurat pe dispozitiv, timpul de procesare pe backend, timpul de răspuns al microserviciului AI și timpul de livrare prin SignalR. Valorile măsurate au fost: timp \textit{dispozitiv -- afișare} de aproximativ [\ \ ] secunde și timp \textit{dispozitiv -- alertă} de aproximativ [\ \ ] secunde, confirmând funcționarea în regim apropiat de timp real.
...
A fost realizat un subsistem de achiziție a datelor de la senzori biomedicali, cu transmitere în timp real către un server centralizat (O1), și a fost dezvoltată o aplicație web completă pentru vizualizarea datelor, accesibilă atât aparținătorilor, cât și medicilor (O2). A fost definit și implementat mecanismul original de evaluare a stării de sănătate --- health score-ul --- bazat pe corelarea mai multor parametri vitali și pe praguri personalizabile (O3), și a fost integrat un sistem de notificări multi-canal declanșat automat (O4). Sistemul funcționează în regim apropiat de timp real, cu o latență redusă (O5), iar arhitectura stratificată și modulară adoptată îi asigură scalabilitatea și extensibilitatea (O6).
...
\item integrarea de senzori suplimentari --- de exemplu, măsurarea ECG sau a tensiunii arteriale --- pentru o evaluare medicală mai cuprinzătoare;
