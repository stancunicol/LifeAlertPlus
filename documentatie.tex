...
Modelul de date al sistemului este alcătuit din unsprezece entități, fiecare corespunzând unei tabele în baza de date. Entitățile principale și rolul lor sunt următoarele:

\begin{itemize}[leftmargin=1.5em]
\item \textbf{User} --- utilizatorul aplicației web (aparținător, medic sau administrator). Conține datele de identificare, credențialele, preferințele de afișare și de notificare (email, push, SMS), precum și un set de praguri implicite pentru parametrii vitali.
\item \textbf{Role} --- rolul unui utilizator, care îi determină drepturile în aplicație.
\item \textbf{Monitored} --- persoana monitorizată (pacientul vârstnic). Conține datele personale, numărul de serie al dispozitivului asociat, frecvența de actualizare și \textbf{pragurile de alertă personalizate} pentru puls, temperatură și \spo.
\item \textbf{UserMonitored} --- tabelă de legătură care modelează relația \textit{mulți-la-mulți} dintre utilizatori și persoanele monitorizate; un utilizator poate monitoriza mai multe persoane, iar o persoană poate fi monitorizată de mai mulți utilizatori. Conține și tipul relației (de exemplu, gradul de rudenie).
...
\item \textbf{Notification} --- o notificare generată de sistem, asociată unei persoane monitorizate și, opțional, unui utilizator destinatar.
...
\subsubsection{Ștergerea logică}

Entitățile importante --- utilizatorii, persoanele monitorizate, notificările --- folosesc mecanismul de \textbf{ștergere logică} (soft delete): în loc de a fi eliminate fizic din baza de date, înregistrările primesc o dată în câmpul \texttt{DeletedAt} și sunt excluse din interogările obișnuite. Această abordare păstrează integritatea istoricului medical și permite recuperarea datelor în caz de eroare.
...
Fluxul principal al sistemului începe pe dispozitivul portabil. La intervalul de timp configurat, microcontrollerul citește valorile tuturor senzorilor, filtrează citirile evident eronate și calculează mărimile derivate. Pachetul de date rezultat este transmis backend-ului printr-o cerere HTTP către endpoint-ul de ingestie, autentificată printr-o cheie de dispozitiv transmisă în antetul cererii. Dacă transmiterea eșuează din cauza lipsei conexiunii, măsurătoarea este adăugată în coada locală a dispozitivului și retransmisă ulterior. La recepție, backend-ul validează cheia de dispozitiv, asociază măsurătoarea persoanei monitorizate pe baza numărului de serie, o persistă în baza de date și o transmite mai departe serviciului de monitorizare a alertelor.
...
Senzorii sunt interogați ciclic, într-o buclă cu perioada de ordinul a câteva sute de milisecunde. Întrucât MAX30100, ecranul OLED și MPU6050 partajează aceeași magistrală I2C, accesul la fiecare dintre ele se face prin selectarea canalului corespunzător al multiplexorului TCA9548A înaintea fiecărei citiri. Senzorul de temperatură DS18B20 comunică separat, prin protocolul 1-Wire, iar modulul GPS NEO-6M prin interfața UART, datele de poziție fiind extrase din mesajele transmise în format NMEA. Valorile brute sunt validate local, prin compararea cu intervalele fizic posibile, iar citirile evident eronate --- generate de un contact imperfect al senzorului cu pielea --- sunt eliminate înainte de a fi luate în considerare. Din componentele de accelerație pe cele trei axe se calculează magnitudinea vectorului de accelerație, mărime derivată folosită ulterior la detectarea imobilității și a căderilor.
...
Datele de conectare la rețeaua WiFi sunt păstrate în memoria nevolatilă a dispozitivului, fiind introduse o singură dată, la configurarea inițială. Această abordare respectă principiul simplității pentru utilizatorul vârstnic, prezentat în subcapitolul 2.1: după configurarea efectuată de aparținător, dispozitivul se reconectează automat la pornire, fără a necesita vreo intervenție tehnică ulterioară din partea pacientului.
...
Pe lângă controlere, backend-ul include servicii de fundal care rulează independent de cereri: serviciul de monitorizare a alertelor și serviciul de raportare zilnică. La pornirea aplicației se execută, de asemenea, popularea inițială a bazei de date cu rolurile și datele de bază.
...
\subsubsection{Validarea cererilor și importul de date}

Fiecare cerere primită de API este validată înainte de a fi procesată: sunt verificate prezența câmpurilor obligatorii, încadrarea valorilor în intervale plauzibile și dreptul utilizatorului de a accesa resursa solicitată. O cerere invalidă este respinsă cu un mesaj explicativ, fără a ajunge la stratul de persistență. Pe lângă datele primite în timp real de la dispozitive, backend-ul oferă și o funcție de \textbf{import}, prin care un istoric de măsurători poate fi încărcat dintr-un fișier extern --- utilă, de exemplu, pentru migrarea datelor dintr-un sistem anterior sau pentru popularea aplicației cu date de test.
...
Pe lângă alertele declanșate de situații de risc, sistemul generează, pentru utilizatorii care au activat această opțiune, un \textbf{raport zilnic} de sănătate. Un serviciu de fundal rulează periodic, agregă măsurătorile zilei --- valori minime, maxime și medii, numărul de căderi și de alerte --- și trimite sinteza prin email. Acest mecanism oferă aparținătorului o imagine de ansamblu chiar și în zilele fără evenimente, fără a fi necesară consultarea activă a aplicației.
