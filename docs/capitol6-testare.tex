...
Testarea cu dispozitivul fizic a inclus și verificarea sistemului de alimentare. S-a constatat că timpul de încărcare completă a bateriei este de aproximativ \textbf{2 ore}, pornind de la o baterie descărcată, în condiții normale de utilizare.
...
Pe lângă testarea funcțională manuală, aplicația dispune de o suită de \textbf{217 de teste automate}, organizate în două categorii, executate automat la fiecare push în pipeline-ul CI/CD configurat cu GitHub Actions.

\subsubsection{Teste unitare}

Testele unitare verifică izolat comportamentul fiecărui serviciu și controler, folosind obiecte mock (prin biblioteca \textbf{Moq}) pentru a elimina dependențele externe (baza de date, email, SMS). Sunt acoperite: \texttt{AuthenticationService} (înregistrare, autentificare, verificare email, resetare parolă), \texttt{JwtService} (generare și validare token), \texttt{UserService}, \texttt{MeasurementService}, \texttt{MonitoredService} (inclusiv operațiile de ștergere logică și reactivare), \texttt{WifiNetworkService} și \texttt{ConditionRuleEngine} (motorul de reguli medicale deterministe). La nivel de controlere REST sunt testate scenariile principale de succes și de eșec pentru \texttt{AuthenticationController}, \texttt{ESPController}, \texttt{MeasurementController}, \texttt{MonitoredController} (logica de ștergere inteligentă: ultimul proprietar, proprietari multipli, administrator) și \texttt{WifiController}.
...
Setul de date utilizat la antrenare și evaluare a fost construit sintetic, pornind de la sursele publice menționate în subcapitolul 2.3. El cuprinde \textbf{8.000 de exemple}: 5.000 corespunzătoare stării NORMAL (extrase și augmentate din sursele publice reale), 1.500 pentru starea CRITICAL și 1.500 pentru starea ALERT, generate sintetic cu valori care respectă pragurile clinice din subcapitolul 4.3, astfel încât modelul să dispună de suficiente cazuri din fiecare clasă. Caracterul stratificat al validării garantează că această proporție este păstrată în fiecare partiție, evitând estimările optimiste care ar putea apărea dacă o clasă rară ar fi insuficient reprezentată în subsetul de testare.
...
\hline
Acuratețe & 1,0000 & 0,0000\\
\hline
Scor F1 (macro) & 1,0000 & 0,0000\\
\hline
Scor F1 (ponderat) & 1,0000 & 0,0000\\
\hline
AUC-ROC (one-vs-rest) & 1,0000 & N/A\\
\hline
\end{tabular}
...
Pentru a verifica absența supraînvățării (overfitting), a fost comparată acuratețea pe datele de antrenare cu cea pe datele de testare; o diferență mică între cele două indică o bună capacitate de generalizare a modelului \textit{(diferența măsurată: 0,0000 — nicio diferență între acuratețea pe antrenare și testare)}.
...
\subsection{Testarea în condiții reale de utilizare}

Dispozitivul a fost purtat de un utilizator sănătos pe o perioadă de aproximativ [\ \ ] ore, cu bateria complet încărcată, pentru a valida funcționarea sistemului cu date reale. Pe durata sesiunii, măsurătorile au fost transmise la intervalul configurat de [\ \ ] secunde, iar tabloul de bord a reflectat valorile în timp real.

Pe toată durata sesiunii de [\ \ ] ore, dispozitivul a rămas funcțional fără ca bateria să se epuizeze, confirmând că autonomia este suficientă pentru un interval de monitorizare continuu de această durată. Senzorul MAX30100 a furnizat citiri stabile pentru frecvența cardiacă și \spo, iar senzorul GY-906 a înregistrat temperatura corporală fără fluctuații semnificative pe durata sesiunii.

Nu au fost declanșate alerte false pe parcursul sesiunii de testare cu date reale, confirmând că pragul de persistență funcționează corect și în afara mediului de simulare.
