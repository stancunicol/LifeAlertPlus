...
\begin{figure}[H]
\centering
\figplaceholder{[Captură de ecran: secțiunea de predicții de tendințe din pagina pacientului, cu etichetele de avertizare, valorile curente, rata de variație și timpul estimat până la prag.]}
...
Atunci când dispozitivul funcționează fără conexiune la internet, măsurătorile se acumulează în coada locală (capacitate maximă: 50 de intrări). La revenirea conexiunii, dispozitivul transmite măsurătorile aflate în așteptare în ordinea înregistrării lor, câte 5 per ciclu de POST, pentru a nu perturba fluxul curent de date. Astfel, istoricul medical rămâne complet chiar și în condiții de conectivitate intermitentă.
...
\begin{figure}[H]
\centering
\figplaceholder{[Spaţiu pentru figură: schema electrică a dispozitivului, prezentând ESP32-C3 Mini, multiplexorul TCA9548A, senzorii MAX30100/MPU6050/GY-906 MLX90614/NEO-6M, ecranul OLED SSD1306, butoanele şi LED-urile, cu pinii GPIO corespunzători şi rezistoarele de pull-up evidenţiate. Schema poate fi exportată din KiCad sau Fritzing.]}
...
\begin{figure}[H]
\centering
\figplaceholder{[Spațiu pentru figură: fotografie a dispozitivului prototip asamblat pe breadboard sau PCB, cu senzorii, multiplexorul TCA9548A, ecranul OLED și cablajele vizibile. Fotografia trebuie realizată pe fundal alb sau neutru, cu lumină uniformă.]}
...
Pe lângă pulsul calculat prin detectarea vârfurilor pe semnalul IR, firmware-ul calculează și saturația de oxigen \spo\ direct pe dispozitiv, fără a transmite valorile brute ADC către server. Algoritmul utilizează raportul \textbf{R} dintre amplitudinea AC și componenta DC a semnalului RED, respectiv IR, obținute prin urmărire exponențial-mobilă (EMA):

\[
R = \frac{(\text{RED}_{AC} / \text{RED}_{DC})}{(\text{IR}_{AC} / \text{IR}_{DC})}
\]

Valoarea \spo\ este estimată printr-un model pătratic empiric, mai precis decât aproximarea liniară clasică $110 - 25 \cdot R$:
...
\subsubsection{OTA --- actualizare firmware prin rețea}

Firmware-ul suportă actualizare \textbf{Over-The-Air (OTA)} prin protocolul HTTPS. La fiecare \textit{heartbeat} (5 minute), dispozitivul interoghează endpoint-ul \texttt{/api/ESP/ota/\{serial\}?version=x.y.z}; dacă serverul returnează \texttt{updateAvailable: true} și un URL valid, firmware-ul descarcă imaginea prin \texttt{esp\_https\_ota} și repornește automat în noua versiune. Dacă descărcarea eșuează, dispozitivul continuă să ruleze versiunea curentă fără a fi afectat. Mecanismul este activat prin configurarea a două variabile în Azure App Settings: \texttt{Urls\_\_OtaFirmwareUrl} (URL-ul binarului .bin) și \texttt{Urls\_\_OtaFirmwareVersion} (versiunea țintă, comparată cu \texttt{FIRMWARE\_VERSION} din cod).
