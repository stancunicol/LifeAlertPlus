...
\begin{tikzpicture}[
  every node/.style={font=\scriptsize},
  esp/.style={rectangle, draw, thick, fill=blue!10,
              minimum width=2.8cm, minimum height=1.2cm, align=center},
  mux/.style={rectangle, draw, thick, fill=orange!15,
              minimum width=2.6cm, minimum height=2.8cm, align=center},
  sens/.style={rectangle, draw, fill=green!12,
               minimum width=3.0cm, minimum height=0.75cm, align=center},
  pwr/.style={rectangle, draw, fill=yellow!20,
              minimum width=2.4cm, minimum height=0.75cm, align=center},
  peri/.style={rectangle, draw, fill=red!8,
               minimum width=2.4cm, minimum height=0.75cm, align=center},
  lbl/.style={font=\tiny},
  arr/.style={-{Latex[length=2mm]}, thick},
  biarr/.style={{Latex[length=2mm]}-{Latex[length=2mm]}, thick, blue!70}]

%% ── Coloana stângă: periferice ──────────────────────────
\node[peri] (gps)  at (0, 5.4) {\textbf{NEO-6M GPS}\\\scriptsize UART, 9600 baud};
\node[peri] (btn1) at (0, 4.2) {\textbf{Buton panică}\\\scriptsize GPIO3, pull-up};
\node[peri] (btn2) at (0, 3.2) {\textbf{Buton sleep}\\\scriptsize GPIO4, pull-up};
\node[peri] (ledY) at (0, 2.2) {\textbf{LED galben}\\\scriptsize GPIO5};
\node[peri] (ledG) at (0, 1.2) {\textbf{LED verde}\\\scriptsize GPIO6};

%% ── Coloana centrală: ESP32-C3 Mini ─────────────────────
\node[esp]  (esp)  at (5.0, 5.4) {\textbf{ESP32-C3 Mini}\\\scriptsize WiFi, RISC-V};
\node[esp]  (uart) at (5.0, 4.2) {\scriptsize UART1\\\scriptsize GPIO1(RX) / GPIO2(TX)};
\node[esp]  (gpio) at (5.0, 3.2) {\scriptsize GPIO3/4/5/6\\\scriptsize butoane \& LED-uri};
\node[esp]  (i2c)  at (5.0, 2.0) {\scriptsize I2C master\\\scriptsize GPIO8(SDA) / GPIO9(SCL)};

%% ── Coloana centrală jos: alimentare ────────────────────
\node[pwr] (lipo)   at (5.0, 0.6) {\textbf{LiPo 3.7\,V / 1500\,mAh}};
\node[pwr] (tp4056) at (5.0,-0.3) {\textbf{TP4056} — încărcare USB-C};
\node[pwr] (mt3608) at (5.0,-1.2) {\textbf{MT3608} — boost 3.3\,V};

%% ── Coloana dreaptă: multiplexor + senzori ──────────────
\node[mux] (mux)  at (10.2, 3.5) {
  \textbf{TCA9548A}\\[2pt]
  \scriptsize I2C mux (0x70)\\[4pt]
  \scriptsize ch\,0: MAX30100\\
  \scriptsize ch\,1: OLED\\
  \scriptsize ch\,2: MPU6050\\
  \scriptsize ch\,3: MLX90614};

\node[sens] (max)  at (14.5, 5.2) {\textbf{MAX30100}\\\scriptsize puls \& \spo\ (0x57)};
\node[sens] (oled) at (14.5, 4.1) {\textbf{OLED SSD1306}\\\scriptsize display (0x3C)};
\node[sens] (mpu)  at (14.5, 3.0) {\textbf{MPU6050}\\\scriptsize IMU (0x68)};
\node[sens] (mlx)  at (14.5, 1.9) {\textbf{GY-906 MLX90614}\\\scriptsize temp. IR (0x5A)};

%% ── Săgeți periferice → ESP ─────────────────────────────
\draw[biarr] (gps.east)  -- node[above,lbl]{UART1} (esp.west |- gps.east);
\draw[arr]   (btn1.east) -- (gpio.west |- btn1.east);
\draw[arr]   (btn2.east) -- (gpio.west |- btn2.east);
\draw[arr]   (gpio.west |- ledY.east) -- (ledY.east);
\draw[arr]   (gpio.west |- ledG.east) -- (ledG.east);

%% ── Săgeți alimentare ───────────────────────────────────
\draw[arr] (lipo.south) -- (tp4056.north);
\draw[arr] (tp4056.south) -- (mt3608.north);
\draw[arr] (mt3608.east) to[out=0,in=270] node[right,lbl]{3.3\,V} (i2c.south);

%% ── Săgeată I2C → MUX ───────────────────────────────────
\draw[biarr] (i2c.east) -- node[above,lbl]{I2C} (mux.west |- i2c.east);
...
\end{tikzpicture}
\caption{Schema de conexiuni a dispozitivului portabil: ESP32-C3 Mini conectat prin I2C la multiplexorul TCA9548A și la cei patru senzori pe canale separate, la modulul GPS prin UART, și la butonul de panică, butonul sleep și LED-uri direct pe GPIO. Circuitul de alimentare: LiPo $\to$ TP4056 (încărcare USB-C) $\to$ MT3608 (boost 3.3\,V).}
\label{fig:schema}
\end{figure}
