...
\titlespacing*{\section}{0pt}{2.0ex plus 0.8ex minus .2ex}{0.5ex plus .2ex}
\titlespacing*{\subsection}{0pt}{0.6ex plus 0.3ex minus .1ex}{0.4ex plus .1ex}
\titlespacing*{\subsubsection}{0pt}{0.5ex plus 0.2ex minus .1ex}{0.3ex plus .1ex}
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[LO]{\small LifeAlertPlus}
\fancyhead[RE]{\small Stancu Nicol-Alexia}
\fancyfoot[C]{\thepage}
...
\begin{tikzpicture}[node distance=0.6cm and 1.2cm,
  ...
  \node[app, below=of ret] (app) {\textbf{Nivelul de aplicație}\\\scriptsize backend ASP.NET Core, microserviciu AI, bază de date PostgreSQL (Neon) și aplicație web Blazor pe Azure};
...
\begin{figure}[H]
\centering
\includegraphics[width=0.72\textwidth]{figuri/interfata_dispozitiv.jpg}
...
\textbf{Notificările push în aplicație (SignalR)}: mesaje livrate instantaneu în interfața web, atâta vreme cât aceasta este deschisă; oferă cea mai mică latență și actualizează tabloul de bord în timp real.

\textbf{Notificările push în browser (Web Push)}: un al patrulea canal complementar, care livrează notificări direct în browser-ul aparținătorului chiar și atunci când aplicația nu este deschisă, prin protocolul Web Push standardizat (VAPID). La prima autentificare, aplicația solicită permisiunea browser-ului; dacă este acordată, subscripția este înregistrată pe server și utilizată ulterior la fiecare alertă. Astfel, aparținătorul este informat și în situațiile în care tabul aplicației este închis sau browserul rulează în fundal.

Combinarea celor patru canale asigură redundanță: dacă un canal eșuează sau nu este observat, un altul preia rolul de informare, crescând probabilitatea ca aparținătorul să fie alertat la timp. Fluxul comparativ al celor trei canale este sintetizat în figura~\ref{fig:multi-canal}.
...
\begin{tikzpicture}[node distance=0.7cm and 1.6cm,
  ...
  \node[chan, right=2.0cm of alerta, yshift=2.4cm] (push) {Push SignalR\\\scriptsize latență mică, aplicația deschisă};
  \node[chan, right=2.0cm of alerta, yshift=0.8cm] (webpush) {Web Push (VAPID)\\\scriptsize browser în fundal};
  \node[chan, right=2.0cm of alerta, yshift=-0.8cm] (sms) {SMS (Twilio)\\\scriptsize acoperire maximă};
  \node[chan, right=2.0cm of alerta, yshift=-2.4cm] (mail) {Email (SMTP)\\\scriptsize conținut bogat, asincron};

  \node[dest, right=1.8cm of sms] (apar) {Aparținător};

  \draw[arrow] (alerta.east) -- (push.west);
  \draw[arrow] (alerta.east) -- (webpush.west);
  \draw[arrow] (alerta.east) -- (sms.west);
  \draw[arrow] (alerta.east) -- (mail.west);
  \draw[arrow] (push.east) -- (apar.west);
  \draw[arrow] (webpush.east) -- (apar.west);
  \draw[arrow] (sms.east) -- (apar.west);
  \draw[arrow] (mail.east) -- (apar.west);
\end{tikzpicture}
\caption{Livrarea alertei prin patru canale complementare asigură redundanța mecanismului de notificare.}
...
Persistența datelor este asigurată de o bază de date relațională \textbf{PostgreSQL}, găzduită în cloud prin serviciul \textbf{Neon} (serverless Postgres), care oferă scalare automată și conectare securizată TLS. Accesul la baza de date se realizează prin intermediul unui ORM (Object-Relational Mapper), anume \textbf{Entity Framework Core} cu driverul \textbf{Npgsql}, care mapează automat clasele din cod pe tabelele bazei de date și permite formularea interogărilor direct în limbajul C\#.
...
Procesul de construire, testare și publicare a aplicației este automatizat printr-un flux de \textbf{integrare și livrare continuă (CI/CD)} configurat cu GitHub Actions: la fiecare push pe ramura principală, testele automate sunt rulate, imaginile Docker sunt construite și publicate pe Docker Hub, iar componentele sunt deployate automat pe \textbf{Microsoft Azure} (App Service pentru backend și client, Container Instances pentru microserviciul AI).
...
Avantajul acestei separări este că logica de business rămâne independentă de tehnologiile concrete: furnizorul de baze de date ar putea fi înlocuit fără a modifica straturile Domain și Application (în producție se folosește PostgreSQL, iar testele de integrare pot rula pe PostgreSQL sau pe SQLite in-memory, configurabil prin variabila de mediu \texttt{TEST\_POSTGRES\_URL}). Totodată, structura facilitează testarea izolată a fiecărui strat și întreținerea pe termen lung.
...
\begin{tikzpicture}[
  ...
  \node[db]  (sql)  at (0,-2.4) {PostgreSQL\\\scriptsize Neon cloud};
...
Cele trei componente software ale sistemului sunt împachetate fiecare într-o imagine Docker proprie și publicate automat pe Docker Hub. Această izolare permite actualizarea, repornirea sau scalarea independentă a fiecărei componente, iar mediul de execuție rămâne identic între dezvoltare și producție.

În producție, deploymentul se face pe infrastructura cloud \textbf{Microsoft Azure}, cu arhitecturi diferite per componentă: \textbf{backend-ul} și \textbf{aplicația web} sunt găzduite pe \textbf{Azure App Service} (Windows, publish direct din GitHub Actions fără Docker), iar \textbf{microserviciul AI} rulează pe \textbf{Azure Container Instances} (container Docker). Baza de date \textbf{PostgreSQL} este externalizată pe \textbf{Neon} (serviciu serverless, independent de infrastructura Azure). Această separare permite actualizarea fiecărei componente independent și asigură că baza de date nu este afectată la repornirea serverelor. Procesul de construire și publicare a imaginilor este automatizat printr-un flux de integrare și livrare continuă (CI/CD): la fiecare modificare a codului, componentele sunt reconstruite și, după validare, publicate automat, ceea ce reduce riscul erorilor manuale și asigură consistența între versiuni.
...
Persistența datelor este asigurată de o bază de date relațională \textbf{PostgreSQL} găzduită în cloud (Neon serverless Postgres), accesată prin ORM-ul Entity Framework Core cu driverul Npgsql. Baza de date stochează utilizatorii, persoanele monitorizate, istoricul măsurătorilor, notificările și datele agregate. Structura sa este detaliată în subcapitolul următor.
...
\item \textbf{Notification}: o notificare generată de sistem, asociată unei persoane monitorizate și, opțional, unui utilizator destinatar. Pe lângă conținutul mesajului, înregistrează și \textbf{feedback-ul utilizatorului} cu privire la veridicitatea alertei. Mecanismul funcționează astfel: atunci când o stare critică a unui vârstnic se rezolvă (parametrii vitali revin la normal după ce o alertă a fost trimisă), ultima notificare critică din acel episod este marcată automat ca \textit{așteptând feedback} (\texttt{FeedbackRequestedAt}). La următoarea accesare a paginii persoanei monitorizate, un banner vizibil solicită confirmarea dacă alerta a fost reală sau o alarmă falsă. Dacă au avut loc mai multe episoade critice, feedback-ul se solicită pentru \textbf{ultima} notificare din fiecare episod, per utilizator. Răspunsul este înregistrat în câmpul \texttt{WasReal}.
...
\item \textbf{DoctorNote}: o notiță medicală adăugată de medicul invitat, asociată unei persoane monitorizate. Conține conținutul notei, data creării și ultimei actualizări, precum și identificatorul și adresa de email a medicului autor. La adăugarea sau actualizarea unei notițe, aparținătorii persoanei monitorizate primesc automat o notificare push în aplicație și prin email.
\item \textbf{PushSubscription}: subscripția unui utilizator la notificările Web Push prin browser (VAPID), stocând endpoint-ul furnizat de browser, cheia publică și secretul de autentificare. Subscripțiile expirate sunt eliminate automat la prima eșuare a livrării.
\item \textbf{AuditLog} și \textbf{SystemError}: jurnale de activitate și de erori ale sistemului, consultabile de administratori prin interfața de administrare.
\end{itemize}
...
Sistemul folosește două politici complementare de ștergere, în funcție de tipul datelor. Pentru entitățile cu rol identitar, anume \texttt{User} și \texttt{Monitored}, se aplică \textbf{ștergere în două etape}: mai întâi o \textbf{ștergere logică} (soft delete), prin care înregistrarea primește o dată în câmpul \texttt{DeletedAt} și este exclusă din interogările obișnuite. Aceasta oferă o \textbf{perioadă de grație de 7 zile} în care un administrator poate reactiva contul sau profilul (ștergând câmpul \texttt{DeletedAt}). Dacă în această perioadă nu are loc nicio reactivare, job-ul zilnic de retenție execută automat \textbf{ștergerea fizică definitivă} (hard delete), eliminând ireversibil înregistrarea și toate datele dependente prin cascadă.

Ștergerea unui profil \texttt{Monitored} este condiționată și de structura de proprietate: deoarece o persoană monitorizată poate fi asociată mai multor aparținători prin tabela \texttt{UserMonitored}, ștergerea logică se aplică \emph{numai} dacă solicitantul este singurul proprietar rămas. Dacă există și alți aparținători, se elimină exclusiv legătura \texttt{UserMonitored} a utilizatorului curent, profilul rămânând activ pentru ceilalți. Administratorul poate aplica ștergerea logică indiferent de numărul proprietarilor, cu posibilitate de reactivare din tabloul de administrare.

Pentru entitățile de tip volumetric, anume \texttt{Measurement}, \texttt{Notification}, \texttt{DailyHistory} și \texttt{WeeklyHistory}, se aplică \textbf{ștergere fizică} (hard delete) prin \textit{job-ul} zilnic de retenție descris în subcapitolul 4.4: înregistrările mai vechi decât perioada configurată (\texttt{Data\-Retention\-Days} de pe persoana monitorizată) sunt eliminate definitiv, atât pentru a respecta principiul minimizării datelor cerut de regulamentul GDPR, cât și pentru a limita creșterea bazei de date sub fluxul continuu de măsurători de la dispozitiv.
...
Scorul rezultat este limitat la intervalul $[0, 100]$, unde 0 reprezintă starea cea mai stabilă și 100 corespunde riscului maxim. În interfața aplicației, valoarea afișată este \textbf{inversată}: se afișează $100 - \text{healthscore}$, astfel încât un scor de 100/100 să reprezinte sănătatea perfectă, mai intuitiv pentru utilizatorul vârstnic și aparținătorul său. Avantajul acestui mecanism este că surprinde \textbf{corelația dintre mai mulți parametri}: mai multe abateri ușoare, fiecare insuficientă pentru a declanșa singură o alertă, se pot cumula într-un scor ridicat, semnalând o degradare globală a stării pacientului care altfel ar trece neobservată.

\subsubsection{Predicția de tendințe}

Pe lângă evaluarea stării instantanee, sistemul oferă o \textbf{predicție de tendințe pe termen scurt}. Serviciul de monitorizare păstrează, pentru fiecare pacient, un buffer circular cu măsurătorile din ultimele \textbf{120 de secunde} (maxim 100 de eșantioane per parametru) și calculează, pentru fiecare parametru vital (puls, temperatură, \spo), media și \textbf{panta} (rata de variație) pe acest interval. Atunci când panta indică o evoluție constantă către o valoare critică, sistemul estimează timpul rămas până la atingerea pragului prin extrapolare liniară și produce o etichetă explicativă.
...
Odată confirmată, alerta este livrată simultan prin \textbf{toate canalele activate} de aparținător (push în aplicație, Web Push browser, email și SMS). Mecanismul de temporizare funcționează astfel:

\begin{itemize}[leftmargin=1.5em]
\item \textbf{Prag de persistență}: starea anormală trebuie să se mențină cel puțin 1 minut (cu minim 2 măsurători consecutive) pentru a genera prima notificare, eliminând valorile tranzitorii cauzate de contact imperfect al senzorului;
\item \textbf{Retrimitere}: dacă starea critică \textbf{persistă}, notificările sunt retrimise pe toate canalele la intervale de \textbf{2 minute}, atâta timp cât starea nu revine la normal;
\item \textbf{Cooldown non-critic}: pentru alertele de nivel non-critic (avertizare), un mecanism de inhibare de 10 minute între retransmiteri evită inundarea destinatarului.
\end{itemize} În cazul alertelor critice, sistemul include în notificare și informații despre cel mai apropiat spital față de poziția pacientului. Întregul flux, de la achiziția măsurătorii până la livrarea alertei, este ilustrat în figura~\ref{fig:flux-alerta}.
...
Pragurile algoritmului au fost alese empiric pe baza literaturii de specialitate pentru detectori de tip prag aplicat accelerometrelor:

\begin{itemize}[leftmargin=1.5em]
\item \textbf{Free-fall} ($|\vec{a}| < 0{,}5\,g$, minim 80\,ms): magnitudinea acceleraţiei coboară sub 0,5\,g, simulând lipsa forței gravitaționale resimțite în cădere liberă. Durata minimă de 80\,ms elimină șocurile mecanice scurte (lovituri, pași), iar cea maximă de 800\,ms filtrează situațiile în care persoana se apleacă lent.
\item \textbf{Impact} ($|\vec{a}| \geq 2{,}5\,g$, la cel mult 800\,ms după free-fall): un vârf brusc de acceleraţie, corespunzând contactului cu solul. Pragul de 2,5\,g este suficient de ridicat pentru a distinge impactul real de mișcările normale de mers (care generează vârfuri de 1,5--2\,g).
\item \textbf{Imobilitate post-impact} ($||\vec{a}| - 1\,g| < 0{,}25\,g$, susținută 1,5\,s): după impact, magnitudinea acceleraţiei rămâne aproape constantă la valoarea gravitaţiei (1\,g), indicând că persoana este imobilizată pe sol. Perioada de 1,5\,s evită falsele confirmări generate de readaptarea posturală imediat după impact.
\end{itemize} La confirmare, task-ul setează un flag pe care bucla principală îl include în următoarea cerere către endpoint-ul de ingestie. Fluxul restant se identifică cu cel al unei măsurători obișnuite, însă serverul ridică automat severitatea la nivel critic atunci când indicatorul de cădere este activ, declanșând livrarea pe toate canalele activate.
...
Parametrii exacti ai mecanismului sunt: pragul de activitate (pacientul se mișcă în mod obișnuit $\geq$ 65\% din timp la ora respectivă), pragul de somn ($\geq$ 70\% din timp pacientul este staționar cu puls $<$ 75\,bpm), fereastra de inactivitate (ultimele 15 minute), iar detecția anomaliei necesită minimum 20 de înregistrări istorice pentru ora respectivă.

La detectarea unei anomalii, serviciul \texttt{ActivityProfileService} generează o notificare push și trimite un email aparținătorului, cu descrierea scenariului detectat și ora producerii. Notificările comportamentale sunt limitate printr-un \textit{cooldown} de 30 de minute între două notificări de același tip pentru același pacient, ca să evite inundarea aparținătorului cu alerte comportamentale repetitive.

\subsubsection{Configurarea rețelelor WiFi de la distanță}

Pentru a permite folosirea dispozitivului în mai multe medii (de exemplu, acasă și pe un hotspot mobil), aparținătorul poate înregistra din aplicația web până la trei rețele WiFi asociate dispozitivului unui pacient. Lista este persistată în baza de date; dispozitivul interoghează periodic backend-ul pentru ultima configurație disponibilă, în paralel cu \textit{heartbeat-ul} obișnuit. Răspunsul conține atât lista de rețele, cât și intervalul de transmitere configurat (\texttt{updateIntervalMs}), derivat din câmpul \textit{UpdateFrequency} al persoanei monitorizate. Atunci când configurația citită diferă de cea salvată local, lista este rescrisă în memoria nevolatilă (evitând uzura memoriei flash) și actualizată și în memorie, astfel încât rețelele noi sunt disponibile la primul reconect, fără a fi necesară repornirea dispozitivului. Intervalul de transmitere este aplicat imediat, fără repornire. Fluxul complet este ilustrat în figura~\ref{fig:wifi-flow}.
...
\begin{tikzpicture}[node distance=1.1cm and 2.0cm,
  ...
  \node[box] (ui) {Aplicația web\\\scriptsize{aparținătorul adaugă SSID + parolă / UpdateFrequency}};
  \node[box, below=of ui] (db) {Baza de date\\\scriptsize{tabela WifiNetworks + câmp UpdateFrequency}};
  \node[box, below=of db] (hb) {ESP, heartbeat (5\,min)\\\scriptsize{GET /api/ESP/wifi-config/\{serial\}}};
  \node[box, below=of hb] (resp) {Răspuns JSON\\\scriptsize{networks[ ] + updateIntervalMs}};
  \node[box, below=of resp] (diff) {Diff vs. NVS local};
  \node[box, below=of diff] (nvs) {NVS rescris + memorie actualizată\\\scriptsize{dacă există diferență — rețele active la primul reconect}};
  \node[box, below=of nvs, fill=green!10] (boot) {updateIntervalMs aplicat imediat\\\scriptsize{fără repornire}};

  \draw[arrow] (ui) -- node[right]{\scriptsize POST} (db);
  \draw[arrow] (db) -- (hb);
  \draw[arrow] (hb) -- (resp);
  \draw[arrow] (resp) -- (diff);
...
\subsubsection{Sincronizarea datelor stocate local și comportamentul la capacitate maximă}

Coada locală este utilizată \textbf{exclusiv atunci când dispozitivul nu are conexiune WiFi}. Atât timp cât WiFi-ul este activ, măsurătorile sunt transmise direct către server, fără a fi stocate în coadă — prin urmare, în condiții normale de funcționare, coada ar trebui să fie goală. Dacă un POST eșuează în timp ce WiFi-ul este conectat (de exemplu, serverul este temporar indisponibil), datele acelui ciclu se pierd, dar ciclul următor va retransmite noile măsurători. La revenirea conexiunii WiFi după o perioadă offline, dispozitivul golește mai întâi coada (câte 5 intrări per ciclu) înainte de a trimite datele noi, astfel că \textbf{istoricul din perioadele offline este recuperat complet}.

Un aspect important al proiectării este comportamentul \textbf{la atingerea capacității maxime a cozii}: dacă cele 50 de locuri sunt ocupate și sosesc noi măsurători (de exemplu, în cazul unei pene de curent prelungite), \textbf{cea mai veche înregistrare din coadă este suprascrisă}. Aceasta înseamnă că, în lipsa conexiunii pentru perioade lungi, se păstrează cele mai recente 50 de măsurători, nu cele mai vechi. Această politică a fost aleasă deliberat: datele recente sunt mai relevante clinic decât cele vechi, iar dimensiunea fixă a cozii protejează memoria flash a dispozitivului împotriva uzurii prin scrieri excesive.

\subsubsection{Arhivarea persoanelor monitorizate}

Aparținătorul poate \textbf{arhiva} o persoană monitorizată atunci când aceasta nu mai necesită monitorizare activă (de exemplu, la transferul permanent într-un centru de îngrijire). Datele istorice sunt păstrate integral, dar persoana nu mai declanșează alerte și nu mai apare în lista activă. O persoană arhivată poate fi \textbf{restaurată} oricând, fără pierderea datelor. Persoanele arhivate sunt afișate pe un tab separat în interfața web, cu posibilitatea de a consulta istoricul lor de măsurători în scop de referință.
...
\subsubsection{Ștergerea conturilor și a profilelor de pacienți}

Ștergerea unui cont de utilizator sau a unui profil de persoană monitorizată se face în două etape, pentru a permite recuperarea în caz de eroare. La solicitarea de ștergere, înregistrarea primește data în câmpul \texttt{DeletedAt} și este exclusă din operațiunile normale ale aplicației (\textbf{ștergere logică}). Aceasta deschide o \textbf{perioadă de grație de 7 zile} în care un administrator poate reactiva contul sau profilul. Dacă în cele 7 zile nu are loc nicio reactivare, job-ul zilnic de curățare execută automat \textbf{ștergerea fizică definitivă} (hard delete), eliminând înregistrarea și toate datele asociate prin cascadă.

Ștergerea unui profil de persoană monitorizată implică o \textbf{logică condiționată de proprietate}, deoarece același profil poate fi asociat mai multor aparținători prin tabela \texttt{UserMonitored}. La apăsarea butonului de ștergere, sistemul evaluează numărul de proprietari și aplică unul din cele trei scenarii:

\begin{itemize}[leftmargin=1.5em]
\item \textbf{Utilizatorul este singurul proprietar} — profilul primește \texttt{DeletedAt} (ștergere logică) și intră în perioada de grație de 7 zile; administratorul îl poate reactiva oricând în această perioadă.
\item \textbf{Există și alți proprietari} — se șterge doar legătura \texttt{UserMonitored} a utilizatorului curent; profilul rămâne activ, monitorizat de ceilalți aparținători.
\item \textbf{Administratorul șterge profilul} — indiferent de numărul de proprietari, profilul intră direct în ștergere logică, cu buton de reactivare disponibil în tabloul de administrare.
\end{itemize}

Aceasta echilibrează dreptul GDPR la ștergere cu securitatea operațională împotriva erorilor umane sau a cererilor neintenționate.

\subsubsection{Configurarea inițială a conexiunii WiFi}

La primirea dispozitivului, aparținătorul configurează rețeaua WiFi printr-o procedură complet autonomă: la prima pornire fără rețele salvate, dispozitivul intră în \textbf{modul SoftAP}, creând hotspot-ul \texttt{LifeAlert-XXXXXX}. Aparținătorul se conectează cu telefonul și accesează \texttt{192.168.4.1} în browser, unde completează SSID-ul și parola rețelei. Credențialele sunt salvate în NVS și dispozitivul repornește automat conectat la rețea. Ulterior, din aplicația web, pot fi adăugate până la trei rețele WiFi suplimentare (acasă, hotspot mobil etc.), sincronizate automat la heartbeat. Detaliile tehnice sunt în subcapitolul 5.1.
...
\hline
8  & I2C SDA (TCA9548) & Pull-up intern activat\\
\hline
9  & I2C SCL (TCA9548) & Pull-up intern activat\\
\hline
\end{tabular}
...
\begin{tikzpicture}[
  every node/.style={font=\scriptsize},
  BOX/.style={rectangle, draw, thick, minimum width=3.0cm,
              minimum height=0.85cm, align=center},
  ESP/.style={rectangle, draw, very thick, fill=blue!10,
              minimum width=3.2cm, minimum height=6.2cm, align=center},
  MUX/.style={rectangle, draw, thick, fill=orange!12,
              minimum width=2.8cm, minimum height=5.4cm, align=center},
  SENS/.style={BOX, fill=green!10, minimum width=3.4cm},
  PERI/.style={BOX, fill=red!7, minimum width=3.0cm},
  PWR/.style={BOX, fill=yellow!18, minimum width=3.0cm},
  lbl/.style={font=\tiny, inner sep=1pt},
  A/.style={-{Latex[length=2.5mm]}, thick},
  BI/.style={{Latex[length=2.5mm]}-{Latex[length=2.5mm]}, thick, blue!60}]

%% ─── Coordonate fixe: 4 rânduri la y = 3.5 / 2.0 / 0.5 / -1.0
%% ─── Coloane: x=0 (periferie), x=5 (ESP centrat), x=10.5 (MUX), x=15.5 (senzori)

%% Col A — periferice (x=0)
\node[PERI] (btn1) at (0, 3.5) {\textbf{Buton panică}\\\tiny GPIO3, pull-up intern};
\node[PERI] (btn2) at (0, 2.0) {\textbf{Buton sleep}\\\tiny GPIO4, pull-up intern};
\node[PERI] (ledY) at (0, 0.5) {\textbf{LED galben}\\\tiny GPIO5, active-HIGH};
\node[PERI] (ledG) at (0,-1.0) {\textbf{LED verde}\\\tiny GPIO6, active-HIGH};

%% Col A — GPS deasupra ESP (x=5, y=5.5)
\node[PERI] (gps)  at (5.0, 5.5) {\textbf{NEO-6M GPS}\\\tiny UART1, 9600 baud};

%% Col B — ESP32-C3 Mini (x=5, centrat la y=1.25)
\node[ESP]  (esp)  at (5.0, 1.25) {
  \textbf{ESP32-C3 Mini}\\[4pt]
  \tiny WiFi 802.11b/g/n\\
  \tiny RISC-V 32-bit\\[6pt]
  \tiny GPIO8 — SDA\\
  \tiny GPIO9 — SCL\\
  \tiny GPIO1 — UART RX\\
  \tiny GPIO2 — UART TX\\
...
%% Col C — TCA9548A (x=10.5, centrat la y=1.25)
\node[MUX]  (mux)  at (10.5, 1.25) {
  \textbf{TCA9548A}\\[4pt]
  \tiny I2C mux (0x70)\\[6pt]
  \tiny ch\,0: MAX30100\\
  \tiny ch\,1: OLED\\
  \tiny ch\,2: MPU6050\\
  \tiny ch\,3: MLX90614};

%% Col D — senzori (x=15.5, aceleași rânduri ca periferiile)
\node[SENS] (max)  at (15.5, 3.5) {\textbf{MAX30100}\\\tiny puls \& \spo{} (0x57)};
\node[SENS] (oled) at (15.5, 2.0) {\textbf{OLED SSD1306}\\\tiny display (0x3C)};
\node[SENS] (mpu)  at (15.5, 0.5) {\textbf{MPU6050}\\\tiny IMU (0x68)};
\node[SENS] (mlx)  at (15.5,-1.0) {\textbf{GY-906 MLX90614}\\\tiny temp. IR (0x5A)};

%% Alimentare — sub ESP (y=-3.0)
\node[PWR] (lipo)   at (1.5, -3.0) {\textbf{LiPo 3.7\,V}\\\tiny 1500\,mAh};
\node[PWR] (tp)     at (5.0, -3.0) {\textbf{TP4056}\\\tiny încărcare USB-C};
\node[PWR] (mt)     at (8.5, -3.0) {\textbf{MT3608}\\\tiny boost 3.3\,V};

%% ── Săgeți Col A → Col B (orizontale, pe rândul lor exact) ──
\draw[A]  (btn1.east) -- node[above,lbl]{GPIO3} (esp.west |- btn1.east);
\draw[A]  (btn2.east) -- node[above,lbl]{GPIO4} (esp.west |- btn2.east);
\draw[A]  (esp.west |- ledY.east) -- node[above,lbl]{GPIO5} (ledY.east);
\draw[A]  (esp.west |- ledG.east) -- node[above,lbl]{GPIO6} (ledG.east);

%% GPS → ESP (vertical)
\draw[BI] (gps.south) -- node[right,lbl]{UART1} (esp.north);

%% ESP → MUX (orizontală la centrul ESP)
\draw[BI] (esp.east) -- node[above,lbl]{I2C} (mux.west);

%% MUX → senzori (orizontale, pe rândul fiecărui senzor)
\draw[A] (mux.east |- max.west)  -- (max.west);
\draw[A] (mux.east |- oled.west) -- (oled.west);
\draw[A] (mux.east |- mpu.west)  -- (mpu.west);
\draw[A] (mux.east |- mlx.west)  -- (mlx.west);

%% Alimentare: lanț orizontal + săgeată verticală spre ESP
\draw[A] (lipo.east) -- (tp.west);
\draw[A] (tp.east)   -- node[above,lbl]{3.7\,V} (mt.west);
\draw[A] (mt.north) -- ++(0,0.6) -- node[above,lbl]{3.3\,V} ++($(esp.south)-(mt.north)-(0,0.6)$) -- (esp.south);

\end{tikzpicture}}
\caption{Schema de conexiuni a dispozitivului portabil: ESP32-C3 Mini conectat prin I2C la multiplexorul TCA9548A și la cei patru senzori pe canale separate, la modulul GPS prin UART, butoanele și LED-urile direct pe GPIO, și circuitul de alimentare LiPo $\to$ TP4056 (USB-C) $\to$ MT3608 (3.3\,V).}
\label{fig:schema}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{figuri/dispozitiv-breadboard.jpg}
\caption{Prototipul dispozitivului portabil asamblat, cu toți senzorii conectați la multiplexorul TCA9548A și la ESP32-C3 Mini.}
\label{fig:prototip}
\end{figure}

\subsubsection{Magistrala I2C şi multiplexarea TCA9548A}

Trei dintre componentele dispozitivului, anume ecranul OLED SSD1306, senzorul MAX30100 şi senzorul MPU6050, comunică prin magistrala I2C. Întrucât MAX30100 are o adresă fixă (0x57), iar OLED-ul şi MPU6050 au adrese predefinite care nu pot fi modificate fără remontare hardware, conectarea directă pe aceeaşi magistrală ar genera, în practică, comportament instabil din cauza încărcării capacitive cumulate şi a interferenţelor între dispozitive cu cerinţe diferite de \textit{timing}.

Soluţia adoptată este utilizarea unui multiplexor I2C \textbf{TCA9548A} pe 8 canale, care expune fiecărui dispozitiv un canal izolat electric. Microcontrollerul selectează canalul activ printr-o scriere unică pe registrul multiplexorului (adresa 0x70), efectuează tranzacţia I2C cu senzorul vizat, apoi închide canalul. Această secvenţă \textit{open / read / close} este aplicată sistematic în firmware pentru a preveni conflictele de adresă atunci când mai mulţi senzori partajează aceleaşi numere de adresă pe magistrale diferite. Canalul 0 a fost alocat senzorului MAX30100, canalul 1 ecranului OLED, canalul 2 senzorului MPU6050, iar canalul 3 senzorului de temperatură GY-906 MLX90614.
...
Întrucât tensiunea nominală a acumulatorului LiPo (3{,}0--4{,}2\,V) este insuficientă pentru alimentarea simultană a tuturor componentelor în regimul lor optim, sistemul include un convertor \textbf{boost MT3608}, capabil să furnizeze până la 28\,V / 2\,A şi configurat în acest caz pentru tensiunea de operare a sistemului (\textbf{3{,}3\,V}), tensiunea necesară microcontrollerului ESP32-C3 şi senzorilor I2C conectaţi la acesta.
...
\begin{figure}[H]
\centering
\figplaceholder{[Spațiu pentru figură: fotografie cu spatele carcasei dispozitivului, evidențiind fereastra optică a senzorului MAX30100 (puls și \spo) și zona de contact a senzorului GY-906 MLX90614 (temperatură corporală).]}
\caption{Spatele carcasei dispozitivului — zonele de contact ale senzorilor de puls, \spo\ și temperatură corporală.}
\label{fig:carcasa-spate}
\end{figure}

\subsubsection{Diagnoza la pornire şi auto-testarea senzorilor}

Pentru ca defectele de montaj ale senzorilor să poată fi identificate fără instrumente externe, firmware-ul execută la pornire o procedură de \textit{sensor functional test}, ale cărei rezultate sunt afişate pe portul serial. Procedura depăşeşte simpla verificare a prezenţei pe magistrală (\textit{I2C probe}) şi efectuează o citire reală de la fiecare senzor: extrage un eşantion de la MPU6050 şi îl validează împotriva valorilor \textit{stuck zero} sau \textit{bus railed} (toţi biţii la 0 sau la 1, semn al unei defecţiuni de comunicaţie); trezeşte senzorul MAX30100 şi citeşte FIFO-ul după un interval suficient pentru acumularea câtorva eşantioane; citeşte temperatura de la senzorul GY-906 MLX90614 şi verifică încadrarea valorii în intervalul fiziologic plauzibil; verifică prezenţa caracterelor NMEA pe portul UART al modulului GPS. Pentru fiecare senzor se afişează verdictul (\texttt{FUNCTIONAL}, \texttt{FAULT} sau \texttt{NOT DETECTED}) alături de valorile efective citite, ceea ce face vizibile deopotrivă defectele de cablaj, contactul slab al senzorului şi erorile de configurare.
...
\subsubsection{Clasificarea activităţii pe dispozitiv}

În paralel cu maşina de stări a detectorului de căderi, acelaşi flux de eşantioane MPU6050 alimentează un \textbf{acumulator de mişcare} folosit pentru a clasifica activitatea purtătorului între două POST-uri consecutive. Pentru fiecare sample se calculează abaterea magnitudinii faţă de gravitaţie, $\delta = | \, |\vec{a}| - 1\,g \, |$, iar valorile sunt agregate într-o structură protejată de mutex: numărul de sample-uri, suma cumulată a abaterilor şi valoarea maximă observată în fereastră. La fiecare ciclu de POST (5\,s, ~250 sample-uri la 50\,Hz), funcţia care construieşte payload-ul JSON face un snapshot atomic al acumulatorului, îl resetează şi clasifică:

\begin{itemize}[leftmargin=1.5em]
\item dacă media abaterii depăşeşte 0,05\,g (mers susţinut peste zgomotul senzorului de ~0,02\,g la repaus), sau
\item dacă vârful izolat depăşeşte 0,20\,g (mişcare scurtă: ridicat în picioare, întors în pat)
\end{itemize}

atunci eticheta de activitate este \texttt{"moving"}; altfel, \texttt{"stationary"}. Sub un minim de 10 sample-uri (MPU oprit, sleep mode), câmpul este omis din JSON, astfel încât backend-ul să trateze absenţa lui ca lipsă de date în loc să interpreteze pe baza unei ferestre incomplete. Eticheta este apoi persistată pentru fiecare măsurătoare în baza de date şi alimentează profilul de activitate zilnică descris în subcapitolul 4.4.

Avantajul acestei soluţii este că reutilizează exact aceleaşi eşantioane pe care detectorul de căderi le citeşte oricum la 50\,Hz, deci costul suplimentar este aproape zero (o operație de incrementare și o comparaţie per sample). În acelaşi timp, fereastra de ~5\,s este suficient de lungă pentru a netezi zgomotul senzorului şi suficient de scurtă pentru ca acumularea de stări (15 minute de inactivitate într-o oră de obicei activă, de exemplu) să rămână reactivă.
...
Atunci când transmiterea eșuează din cauza lipsei conexiunii, măsurătoarea este salvată într-o \textbf{coadă locală} păstrată în memoria nevolatilă (NVS) a microcontrollerului. Coada are o capacitate maximă de \textbf{50 de înregistrări}; la depășire, cele mai vechi sunt suprascrise. La revenirea conexiunii, înregistrările se transmit treptat, câte \textbf{5 per ciclu de POST}, pentru a nu suprasolicita rețeaua. Alertele de panică sunt stocate într-o \textbf{coadă separată} cu capacitate maximă de 10 intrări și au prioritate la retransmitere față de măsurătorile obișnuite. Intervalele de funcționare sunt: citire senzori la fiecare \textbf{500\,ms}, POST date la fiecare \textbf{5\,s} și heartbeat la fiecare \textbf{5 minute}.

Backend-ul aplică și o \textbf{gardă la ingestie}: dacă persoana monitorizată asociată serialului este arhivată sau se află în perioada de grație de ștergere logică (\texttt{IsArchived} sau \texttt{DeletedAt} setat), cererea de ingestie și alertele de panică sunt acceptate cu \texttt{200 OK} dar datele \emph{nu sunt persistate} și nu declanșează alerte, evitând astfel poluarea bazei de date cu măsurători ale unui profil inactiv.

Structura exactă a payload-ului JSON transmis de dispozitiv la endpoint-ul de ingestie (\texttt{POST /api/esp/data}) este prezentată în tabelul~\ref{tab:esp-payload}; payload-ul heartbeat (\texttt{POST /api/esp/heartbeat}) în tabelul~\ref{tab:esp-heartbeat}.
...
\hline
\textbf{Câmp JSON} & \textbf{Tip} & \textbf{Semnificație} \\
\hline
\texttt{serial}       & string   & Numărul de serie al dispozitivului \\
\texttt{date}         & long     & Timestamp Unix (secunde UTC) al măsurătorii \\
\texttt{bpm}          & int?     & Frecvența cardiacă (bătăi/minut); \texttt{null} dacă senzorul nu returnează date valide \\
\texttt{spo2}         & int?     & Saturația de oxigen (\%), calculată pe dispozitiv \\
\texttt{temperature}  & double?  & Temperatura corporală (\textdegree C), citită de senzorul MLX90614 \\
\texttt{neo6m}        & string?  & Coordonate GPS „latitudine,longitudine" (ex. „44.4268,26.1025"); \texttt{null} dacă fix-ul GPS nu este disponibil \\
\texttt{isFall}       & bool     & \texttt{true} dacă algoritmul de detecție a căderilor a confirmat un eveniment \\
\texttt{battery}      & double?  & Tensiunea bateriei (V), estimată din ADC \\
\texttt{activity}     & string?  & Eticheta de activitate clasificată pe dispozitiv: \texttt{"moving"} sau \texttt{"stationary"} \\
\texttt{mpu6050}      & int[3]   & Vector accelerometru brut [X, Y, Z] (unități ADC) \\
\texttt{gyro}         & int[3]   & Vector giroscop brut [X, Y, Z] \\
\texttt{max30100}     & int[]?   & Date brute MAX30100 (opțional, scop diagnostic) \\
\texttt{errorMessage} & string?  & Mesaj de eroare firmware; prezent doar la eșec de citire \\
\hline
\end{tabular}
\caption{Câmpurile payload-ului JSON transmis de dispozitiv la \texttt{POST /api/esp/data}.}
\label{tab:esp-payload}
\end{table}
...
\hline
\texttt{serial}              & string & Numărul de serie al dispozitivului \\
\texttt{rssiDbm}             & int    & Puterea semnalului WiFi (dBm) \\
\texttt{freeHeapBytes}       & int    & Memoria heap liberă (bytes) \\
\texttt{uptimeSeconds}       & long   & Timp de funcționare neîntreruptă (secunde) \\
\texttt{queuedMeasurements}  & int    & Numărul de măsurători în coada offline \\
\texttt{batteryVoltage}      & float  & Tensiunea bateriei (V) \\
\texttt{sensorFlags}         & int    & Bitmask cu starea senzorilor (0\,=\,toți OK) \\
\texttt{firmwareVersion}     & string & Versiunea firmware-ului (ex. „1.0.0") \\
\hline
\end{tabular}
\caption{Câmpurile payload-ului JSON al mesajului de heartbeat (\texttt{POST /api/esp/heartbeat}).}
\label{tab:esp-heartbeat}
\end{table}

\subsubsection{Calculul saturației de oxigen (\texorpdfstring{\spo}{SpO2}) în firmware}

Pe lângă pulsul calculat prin detectarea vârfurilor pe semnalul IR, firmware-ul calculează și saturația de oxigen \spo\ direct pe dispozitiv. Principiul fizic se bazează pe faptul că hemoglobina oxigenată și cea dezoxigenată absorb diferit lumina roșie și infraroșie: raportul dintre absorbțiile celor două lungimi de undă se modifică în funcție de concentrația de oxigen din sânge.

Senzorul MAX30100 emite alternativ lumină RED (roșie, 660\,nm) și IR (infraroșu, 940\,nm) și măsoară intensitatea recepționată. Fiecare semnal conține o componentă \textbf{DC} (constantă, dată de țesuturile statice) și o componentă \textbf{AC} (pulsatilă, generată de fluxul sanguin). Algoritmul urmărește componenta DC a fiecărui semnal prin medie exponențial-mobilă (EMA), extrage amplitudinea AC (diferența max-min per bătaie cardiacă) și calculează raportul:

\[
R = \frac{(\text{RED}_{AC} / \text{RED}_{DC})}{(\text{IR}_{AC} / \text{IR}_{DC})}
\]

Când sângele este bine oxigenat, hemoglobina absoarbe mult RED și puțin IR ($R$ mic); când saturația scade, absorbția RED scade și cea IR crește ($R$ mare). Valoarea \spo\ este estimată dintr-un \textbf{model pătratic empiric}, calibrat pe datele senzorului MAX30100, mai precis decât aproximarea liniară clasică $110 - 25 \cdot R$:

\[
\text{SpO}_2 = -45{,}060 \cdot R^2 + 30{,}354 \cdot R + 94{,}845
\]

Rezultatul este limitat la intervalul $[70, 100]$\%, cu valoarea 0 indicând absența degetului pe senzor. Calculul se realizează o dată per bătaie cardiacă detectată (la fiecare vârf IR), asigurând o estimare stabilă și sincronizată cu ciclul cardiac. Curentul LED-ului IR este ajustat adaptiv la fiecare ~5 secunde: dacă semnalul este prea slab (IR $<$ 5\,000 unități ADC), curentul crește la 37\,mA; dacă este prea puternic (IR $>$ 40\,000), scade la 11\,mA; altfel rămâne la 27\,mA implicit. Această ajustare reduce consumul de curent fără a sacrifica acuratețea semnalului.

\begin{figure}[H]
\centering
\includegraphics[width=0.55\textwidth]{../figuri/dispozitiv-offline.jpeg}
\caption{Ecranul OLED al dispozitivului: pulsul, \spo\ și temperatura sunt afișate în timp real; SpO2 înlocuiește câmpul de baterie nefuncțional.}
\label{fig:oled-display}
\end{figure}

\subsubsection{Watchdog și recuperarea automată}

Pentru a garanta funcționarea continuă fără supraveghere, firmware-ul activează \textbf{Task Watchdog Timer (TWDT)} cu un timeout de 30 de secunde. Dacă bucla principală îngheață, de exemplu din cauza unui timeout I2C sau a unui blocaj de semaphore, watchdog-ul resetează microcontrollerul automat, fără a necesita intervenție fizică. Buca principală apelează \texttt{esp\_task\_wdt\_reset()} la fiecare ciclu de 500\,ms, confirmând că sistemul rulează normal.

\subsubsection{Timeout GPS și economisire curent}

Modulul GPS NEO-6M consumă aproximativ 30\,mA permanent, inutil în condiții de interior unde fixul GPS nu poate fi obținut. Firmware-ul implementează un \textbf{timeout de 5 minute}: dacă nu se obține niciun fix GPS valid în acest interval, driverul UART al modulului este oprit, economisind curentul. La fiecare \textit{heartbeat}, driverul este reactivat temporar pentru o nouă încercare (de exemplu, dacă pacientul a ieșit în exterior). Dacă un fix valid este obținut, coordonatele sunt incluse în payload-ul JSON; altfel, câmpul \texttt{neo6m} rămâne \texttt{null}.

\subsubsection{Interfața locală}

Interacțiunea cu utilizatorul se realizează prin ecranul OLED, care afișează în timp real pulsul, \spo, temperatura, starea rețelei și numărul de măsurători aflate în coada offline. LED-ul verde indică funcționarea normală, iar cel galben semnalează erori, lipsa rețelei sau modul sleep. Butoanele de panică și de sleep au debounce software de 50\,ms și o cale de polling paralel cu întreruperea GPIO, pentru fiabilitate maximă.

\subsubsection{Configurarea rețelei — modul de provizionare SoftAP}

La prima pornire, dacă nu există nicio rețea WiFi salvată în memoria nevolatilă (NVS), dispozitivul intră automat în \textbf{modul de provizionare}: pornește propriul hotspot WiFi cu SSID-ul \texttt{LifeAlert-XXXXXX} (ultimele 6 caractere din serialul MAC), fără parolă, și servește un server HTTP pe adresa \texttt{192.168.4.1}. Aparținătorul se conectează cu telefonul la acest hotspot și accesează adresa în browser, unde apare un formular simplu de introducere a SSID-ului și a parolei rețelei de acasă. La trimiterea formularului, credențialele sunt salvate în NVS și dispozitivul se repornește automat, conectându-se la rețeaua configurată.

\begin{figure}[H]
\centering
\includegraphics[width=0.55\textwidth]{../figuri/conectare-esp.jpeg}
\caption{Pagina de provizionare WiFi afișată de dispozitiv la adresa \texttt{192.168.4.1}: aparținătorul introduce SSID-ul și parola rețelei de acasă direct din browser-ul telefonului.}
\label{fig:prov-wifi}
\end{figure}

Ulterior, aparținătorul poate înregistra prin aplicația web până la trei rețele WiFi suplimentare, transmise dispozitivului prin mecanismul de sincronizare la \textit{heartbeat} descris în subcapitolul anterior. La fiecare pornire, dispozitivul încearcă pe rând rețelele salvate în NVS; dacă niciuna nu răspunde în 12 secunde, revine la modul de provizionare SoftAP.
...
Autentificarea este gestionată prin token-uri JWT, validate la fiecare cerere pe baza unei chei secrete configurate exclusiv în variabilele de mediu ale serverului, nu în codul sursă. Aplicația acceptă și autentificarea prin OAuth folosind un cont Google. O particularitate de implementare o reprezintă conexiunile SignalR, care transmit token-ul prin șirul de interogare al adresei; backend-ul tratează explicit acest caz.

Securitatea este întărită prin mai multe mecanisme complementare:

\begin{itemize}[leftmargin=1.5em]
\item \textbf{Limitarea ratei} (\textit{rate limiting}) restricționează numărul de cereri de autentificare de la același client, atenuând atacurile de tip forță brută; pentru endpoint-urile IoT, un limitator separat permite maxim \textbf{4 cereri per 60 de secunde per număr de serie}, prevenind inundarea serverului de la un dispozitiv defect;
\item \textbf{Antete HTTP de securitate} (\texttt{X-Content-Type-Options}, \texttt{X-Frame-Options}, \texttt{Referrer-Policy}, \texttt{X-XSS-Protection}) reduc riscul atacurilor de tip \textit{clickjacking} și XSS;
\item \textbf{Politica CORS} restrictivă permite accesul la API doar din domeniul aplicației web autorizate; în producție, HTTPS Only este activat pe Azure App Service, astfel că orice cerere HTTP este redirecționată automat la HTTPS;
\item \textbf{Autentificarea dispozitivului ESP} (\texttt{X-Device-Key}): toate endpoint-urile expuse dispozitivelor IoT sunt protejate printr-o schemă de autentificare per-dispozitiv, separată de sistemul JWT; un secret comun (\texttt{EspDeviceSecret}) este configurat exclusiv în Azure App Settings și în firmware, dar nu este transmis niciodată în rețea; în schimb, fiecare dispozitiv calculează la pornire o cheie derivată \texttt{HMAC-SHA256(secret,\,serial)} și o trimite în antetul \texttt{X-Device-Key}; serverul recalculează independent același HMAC folosind serialul din cerere și le compară printr-o comparație în timp constant (\texttt{CryptographicOperations.FixedTimeEquals}); astfel, compromiterea unui singur dispozitiv nu expune celelalte dispozitive;
\item \textbf{Ștergere logică și fizică}: la solicitarea de ștergere, entitățile cu rol identitar (utilizatori, persoane monitorizate) primesc marcajul \texttt{DeletedAt} și sunt excluse din operațiunile normale ale aplicației (\textit{soft delete}), deschizând o perioadă de grație de 7 zile în care un administrator poate reactiva înregistrarea; la expirarea acesteia, job-ul zilnic de curățare execută \textbf{ștergerea fizică definitivă} (\textit{hard delete}) a înregistrării și a tuturor datelor asociate prin cascadă, echilibrând dreptul GDPR la ștergere cu securitatea operațională;
\item \textbf{VAPID pentru Web Push}: payload-ul notificărilor Web Push este criptat end-to-end între server și browser, prin perechea de chei asimetrice VAPID; subscripțiile sunt unice per browser și eliminate automat la expirare;
\item \textbf{Gestionar global de excepții}: erorile neprevăzute returnează un mesaj generic, fără detalii interne; excepțiile sunt jurnalizate intern prin \texttt{AuditService} și vizibile administratorului în pagina de jurnal de erori;
\item \textbf{Accesul bazat pe roluri}: fiecare endpoint verifică rolul utilizatorului (User, Admin) și dreptul de acces la resursa specifică; medicii invitați au acces exclusiv în regim de citire la pacientul pentru care au acceptat invitația.
\end{itemize}

\subsubsection{Serviciile de fundal}

API-ul include endpoint-uri noi față de versiunea inițial proiectată. \textbf{Notițele medicale} (DoctorNote) sunt gestionate printr-un controler dedicat: medicul invitat poate adăuga sau actualiza o notiță per pacient, iar la fiecare salvare aparținătorii primesc notificare push în aplicație și prin email. \textbf{Subscripțiile Web Push} sunt gestionate de \texttt{PushController}, care expune endpoint-urile de abonare/dezabonare și un endpoint public pentru cheia VAPID. La pornire, \textbf{SimulationManager} este pre-populat cu ultima măsurătoare din baza de date pentru fiecare dispozitiv activ, astfel încât aplicația să afișeze date imediat după un restart al serverului, fără a aștepta o nouă transmitere de la ESP.

Pe lângă controlere, backend-ul include mai multe servicii de fundal care rulează independent de cereri. \textbf{Serviciul de monitorizare a alertelor} (\texttt{AlertMonitorService}) primește fiecare nouă măsurătoare, ține un buffer cu valorile din ultimele minute pentru fiecare pacient și decide momentul exact de trimitere a notificărilor. În paralel, \textbf{serviciul de profilare a activităţii} (\texttt{ActivityProfileService}) construieşte la prima nevoie un profil orar per pacient din ultimele 7 zile de măsurători şi verifică, la fiecare nouă măsurătoare, dacă eticheta de activitate primită de la dispozitiv abate de la tiparul obişnuit; abaterile semnificative declanşează notificările comportamentale descrise în subcapitolul 4.4. \textbf{Serviciul de raportare zilnică} (\texttt{Daily\-Report\-Background\-Service}) rulează o dată pe zi, la miezul nopții, și trimite prin email un raport sintetic al zilei precedente utilizatorilor care au activat această opțiune în setări. \textbf{Serviciul de retenție a datelor} (\texttt{RetentionCleanupBackgroundService}) rulează tot zilnic și execută ștergerea fizică a măsurătorilor, notificărilor și agregărilor mai vechi decât perioada de retenție configurată pentru fiecare pacient. La pornirea aplicației se execută, de asemenea, popularea inițială a bazei de date cu rolurile și datele de bază.
...
Microserviciul de inteligență artificială a fost implementat în limbajul \textbf{Python}, folosind framework-ul web \textbf{FastAPI}. La pornire, serviciul încarcă din fișierul de artefacte modelul Random Forest antrenat, împreună cu obiectele auxiliare necesare (scalatorul Min-Max și codificatorul de etichete), serializate anterior cu biblioteca joblib. Procesul de antrenare și evaluare a modelului a fost realizat în mediul \textbf{Google Colab}, care a oferit acces la resurse GPU fără configurare locală și a facilitat documentarea pas cu pas a experimentelor într-un notebook partajabil. Integrarea inteligenței artificiale în sisteme de monitorizare medicală reprezintă o direcție activă de cercetare, cu dovezi tot mai solide privind capacitatea modelelor de a detecta devierile parametrilor vitali înaintea apariției evenimentelor clinice \cite{maswadi2026, panteli2025, panahi2025future}.

\subsubsection{Antrenarea modelului}

Modelul de inteligență artificială a fost antrenat separat de aplicație, printr-un proces reproductibil executat în Google Colab. Sunt încărcate mai întâi cele trei seturi de date publice menționate în subcapitolul 2.3, pe baza cărora este generat un \textbf{set de date sintetic} de \textbf{8\,000 de înregistrări}: \textbf{5\,000 exemple NORMAL} (extrase și augmentate din sursele publice reale), \textbf{1\,500 exemple CRITICAL} și \textbf{1\,500 exemple ALERT} (generate sintetic cu valori care respectă pragurile clinice). Distribuția inegală față de clasa normală a fost menținută intenționat, întrucât stările de alertă sunt în mod natural mai rare decât starea normală. Setul sintetic este împărțit în \textbf{6\,400 de exemple pentru antrenare} (80\%) și \textbf{1\,600 de exemple pentru testare} (20\%), cu stratificare pentru a păstra proporțiile claselor în ambele subseturi. Componentele de mișcare sunt normalizate cu un scalar Min-Max, iar modelul Random Forest este antrenat și evaluat.

Un exemplu reprezentativ dintr-un rând al setului de date sintetic este prezentat în tabelul~4.
...
\begin{tabular}{|l|c|}
\hline
\textbf{Câmp} & \textbf{Valoare exemplu}\\
\hline
Puls (bpm) & 92\\
\hline
SpO\textsubscript{2} (\%) & 97\\
\hline
Temperatură (°C) & 36,8\\
\hline
AccelX (LSB) & 420\\
...
\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{../figuri/login.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{../figuri/register.png}
...
Accesul la pagini este controlat în funcție de rolul utilizatorului: paginile de administrare sunt disponibile doar administratorilor, iar fiecare aparținător sau medic vede exclusiv persoanele monitorizate la care are drept de acces. Tentativele de acces neautorizat sunt redirecționate către pagina de autentificare. Interfața oferă posibilitatea selectării limbii (română sau engleză), comutarea aplicându-se imediat pe toate paginile. De asemenea, aplicația include un \textbf{asistent conversațional integrat} care răspunde exclusiv la întrebări în legătură directă cu aplicația, pragurile semnelor vitale și afecțiunile medicale monitorizate, fără a ieși din contextul domeniului.

\subsubsection{Organizarea interfeței și gestionarea stării}

Interfața este construită din componente Blazor reutilizabile, anume antetul, widget-urile și notificările de tip \textit{toast}, combinate în pagini. Starea partajată între pagini, precum datele utilizatorului autentificat sau persoana monitorizată selectată, este menținută de servicii dedicate, înregistrate în containerul de injecție de dependențe al aplicației client. Astfel, informațiile încărcate o singură dată sunt disponibile în întreaga aplicație, fără a fi nevoie de cereri repetate către server la fiecare schimbare de pagină. Pagina principală a aparținătorului este lista persoanelor monitorizate, unde fiecare pacient apare sub forma unui card cu starea curentă a parametrilor vitali, ultima actualizare, indicatorul GPS și statusul detectorului de căderi.

Tot din această pagină se accesează \textbf{modalul de editare a profilului pacientului}, prin care aparținătorul configurează pragurile personalizate ale fiecărui parametru vital, frecvența de actualizare a dispozitivului și perioada de retenție a datelor. Suplimentar, modalul de \textbf{configurare a rețelelor WiFi} permite înregistrarea a până la trei rețele pe care dispozitivul portabil le va încerca la conectare.

Pagina de detalii a pacientului include, pe lângă graficele de puls și temperatură, un al treilea grafic pentru saturația de oxigen (culoare albastru, interval 85--100\%), disponibil atât în vizualizarea zilnică, cât și în cea săptămânală, pentru aparținători și medici deopotrivă. Medicul invitat poate adăuga \textbf{notițe medicale} direct din aceeași pagină, vizibile ulterior și aparținătorului; la fiecare salvare, aparținătorii primesc notificare push și email. Dacă ultima măsurătoare are mai mult de 15 minute vechime, un banner de avertizare semnalează că dispozitivul poate fi offline. Pagina cu persoanele monitorizate include și un câmp de căutare live care filtrează cardurile după nume.

\textbf{Onboarding la prima conectare}: la primul login, un modal de bun venit prezintă cei trei pași de configurare inițială (achiziție dispozitiv, configurare WiFi prin SoftAP, adăugare pacient). Modalul nu reapare la sesiunile ulterioare (starea este persistată în \texttt{localStorage}).

\textbf{Stare goală pentru utilizatori noi}: pe tabloul de bord, utilizatorii fără niciun pacient adăugat văd o stare goală informativă cu fotografia dispozitivului, pașii de configurare și adresa de email de suport pentru achiziție.

\textbf{Layout responsiv pe mobil}: paginile de detalii ale pacientului și graficele se adaptează la ecrane mici prin reordonare verticală, fără scroll orizontal excesiv.

\subsubsection{Vizualizarea datelor și funcționalitatea PWA}

Reprezentarea grafică a istoricului parametrilor vitali este realizată direct în SVG, prin curbe Bézier monotone (Fritsch-Carlson) trasate peste o grilă cu axă temporală. Cele trei grafice (puls, temperatură și \spo) sunt așezate într-un container cu \textbf{scroll orizontal sincronizat} și un control de \textbf{zoom} (1x--4x): wrapper-ul rămâne la lățimea cardului, iar SVG-ul intern se lungește, astfel încât citirile dense să nu se mai suprapună (figura~\ref{fig:grafice-vitale}). La trecerea mouse-ului peste oricare zonă a graficului, un tooltip arată valoarea exactă și ora la cel mai apropiat punct înregistrat.

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{../figuri/grafice-vitale.png}
\caption{Graficele zilnice de puls și temperatură, cu tooltip-ul activ la hover (afișează valoarea, de exemplu 82~bpm la 19:16, și ora) și scrollbar orizontal sincronizat sub fiecare grafic.}
\label{fig:grafice-vitale}
\end{figure}

Localizarea pacientului în scenarii de alertă este realizată tot prin interoperabilitate JavaScript, integrând harta Google Maps direct în vizualizarea persoanei monitorizate. Marcatorul este actualizat automat la fiecare nouă măsurătoare cu fix GPS valid. Pentru a evita expunerea cheii API în codul client, aceasta nu este inclusă în pachetul aplicației, ci este obținută la nevoie de la backend printr-un endpoint dedicat (servit doar către cereri autentificate), apoi predată bibliotecii de hărți. Exportul rapoartelor în format PDF folosește același mecanism de interoperabilitate.
...
\subsubsection{Tabloul de bord și paginile aparținătorului}

Pagina de start a aparținătorului după autentificare este \textbf{tabloul de bord} (figura~\ref{fig:dashboard}), care oferă o privire de ansamblu: numărul total de persoane monitorizate, câte se află în stare de alertă, câte sunt stabile, câte sunt offline și numărul de măsurători din ziua curentă, urmate de carduri rezumat pentru cele mai recent actualizate persoane. Tabloul de bord se reîmprospătează periodic, astfel încât starea afișată să rămână actuală fără reîncărcarea manuală a paginii.

Pagina de \textbf{notificări} listează cronologic alertele și mesajele primite, cu posibilitatea de filtrare după tip și de marcare ca citite; un indicator în antet semnalează notificările necitite. Pagina de \textbf{istoric al măsurătorilor} prezintă tabelar măsurătorile brute ale unei persoane, paginat, util pentru o consultare detaliată dincolo de reprezentarea grafică.

Pagina de \textbf{profil} permite utilizatorului să își editeze datele personale, numărul de telefon, adresa de email (cu confirmare prin email nou) și parola, iar pagina de \textbf{setări} grupează preferințele de cont: limba interfeței, preferințele de notificare (descrise în subcapitolul 5.5), pragurile vitale implicite și prima zi a săptămânii folosită în graficele săptămânale.

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{../figuri/utilizator-dashboard.png}
\caption{Tabloul de bord al aparținătorului, cu statistici de ansamblu și carduri pentru persoanele recent actualizate.}
\label{fig:dashboard}
\end{figure}

\subsubsection{Paginile informative și suportul}

Aplicația include și două pagini cu rol informativ, accesibile din meniul principal. Pagina \textbf{Despre noi} prezintă, sub formă de carduri, misiunea proiectului, problema pe care o adresează și principalele caracteristici. Pagina de \textbf{Suport} oferă o imagine a dispozitivului, datele de contact prin email, o listă de verificări de bază și o secțiune de întrebări frecvente cu soluții pentru problemele comune.

\subsubsection{Interfața de administrare}

Pe lângă interfața destinată aparținătorilor și medicilor, aplicația include un set de pagini accesibile exclusiv rolului de \textbf{administrator}, protejate prin verificarea rolului din token-ul JWT la fiecare navigare. Acestea cuprind:

\begin{itemize}[leftmargin=1.5em]
\item \textbf{Tabloul de bord de administrare}, care prezintă statistici agregate la nivelul întregului sistem: numărul total de utilizatori, persoane monitorizate, dispozitive online/offline, alerte active și măsurători din ziua curentă.
\item \textbf{Gestionarea utilizatorilor}, prin care administratorul poate vizualiza conturile, le poate edita sau șterge și poate consulta persoanele monitorizate asociate fiecăruia.
\item \textbf{Statusul dispozitivelor}, un tabel cu toate persoanele monitorizate din sistem, indicând starea fiecăreia (activă, arhivată sau în curs de ștergere). Înregistrările marcate cu \texttt{DeletedAt} sunt evidențiate vizual printr-un badge \textit{Ștergere în curs} și un buton de \textbf{Reactivare}, prin care administratorul poate anula ștergerea logică pe durata perioadei de grație de 7 zile.
\item \textbf{Jurnalul de activitate} și \textbf{jurnalul de erori}, două vizualizări tabelare cu filtrare și căutare, destinate urmăririi evenimentelor de cont și de sistem, respectiv a erorilor și avertismentelor backend-ului.
\end{itemize}

\subsubsection{Arhivarea persoanelor monitorizate}

Aparținătorul poate \textbf{arhiva} o persoană monitorizată atunci când aceasta nu mai necesită monitorizare activă (de exemplu, la transferul permanent într-un centru de îngrijire). Datele istorice sunt păstrate integral, dar persoana nu mai apare în lista activă și nu mai declanșează alerte. O persoană arhivată poate fi \textbf{restaurată} oricând, fără pierderea datelor. Funcționalitatea este accesibilă prin butoane dedicate pe cardul persoanei, cu confirmare printr-un modal înainte de execuție. Persoanele arhivate sunt afișate pe un tab separat, fără indicatori de stare (online/offline).

Persoanele aflate în \textbf{ștergere logică} (câmpul \texttt{DeletedAt} este setat) sunt tratate identic cu cele arhivate din perspectiva interfeței: pagina de detalii devine \textbf{read-only} — nu se mai afișează statusul vital curent, butoanele de editare a profilului, de invitare a medicului sau de configurare WiFi sunt ascunse, iar datele primite de la dispozitiv nu sunt persistate (controllerul ESP returnează \texttt{200 OK} fără a salva măsurătoarea). Un banner roșu afișat în partea superioară a paginii indică faptul că persoana este programată pentru ștergere definitivă și arată numărul de zile rămase din perioada de grație. Administratorul poate anula ștergerea din tabloul de administrare pe durata acestei perioade.

\subsubsection{Fluxul de acceptare a invitației de medic}

Medicul invitat primește prin email un link unic cu token de expirare. Accesând link-ul, vede datele de bază ale pacientului și poate accepta sau refuza accesul (figura~\ref{fig:invitatie-doctor}). Accesul se realizează exclusiv prin link-ul cu token, fără a fi necesară crearea unui cont. Din aceeași pagină, medicul poate adăuga direct o notiță medicală. Invitațiile expirate sau deja folosite returnează un mesaj de eroare specific.

\begin{figure}[H]
\centering
\includegraphics[width=0.80\textwidth]{../figuri/invitatie-doctor.png}
\caption{Pagina de acceptare a invitației de medic: medicul vede datele de bază ale pacientului și poate accepta sau refuza accesul printr-un link unic cu token de expirare, fără a necesita un cont.}
\label{fig:invitatie-doctor}
\end{figure}

\subsubsection{Consimțământul GDPR la înregistrare}

La crearea unui cont nou, utilizatorul bifează o \textbf{căsuță de consimțământ} care acoperă atât prelucrarea datelor personale (Art. 6 GDPR), cât și datele speciale de sănătate ale persoanelor monitorizate (Art. 9 GDPR). Fără bifarea acesteia, înregistrarea nu poate fi finalizată. Consimțământul este stocat cu marcaj temporal în baza de date.

\subsubsection{Editarea contului și schimbarea numărului de telefon}

Pagina de profil permite utilizatorului să modifice numele, numărul de telefon, adresa de email (cu confirmare prin email trimis la noua adresă) și parola. Utilizatorii nu pot încărca o fotografie de profil; în locul acesteia, interfața afișează inițialele numelui pe un fundal colorat, generat automat pe baza numelui contului.

\subsubsection{Pagina de simulări (administrator)}

Administratorii pot porni \textbf{simulări} de date pentru orice dispozitiv, fără a necesita hardware real: datele generate includ o fază de alertă (primele 3 minute, cu valori ce depășesc pragurile) urmată de o fază normală. Pagina include un avertisment vizibil că simulările suprascriu datele reale primite de la dispozitivele fizice, pentru a preveni confuzia în producție.

\subsubsection{Vizualizarea medicului}

Odată acceptată invitația, medicul accesează o pagină dedicată cu aceleași vizualizări ca aparținătorul (vitale curente, grafice, profil de activitate, notițe medicale), dar fără funcțiile de editare a profilului, a pragurilor sau a rețelelor WiFi. Exportul datelor pacientului este posibil în două moduri: medicul poate genera direct un raport PDF din pagina de invitație (accesibilă prin token, fără autentificare), sau aparținătorul poate trimite prin email un export structurat al măsurătorilor pentru un interval de timp selectabil, util evaluării clinice în afara aplicației.

\subsection{Sistemul de notificări}

Sistemul de notificări implementează alertarea multi-canal descrisă conceptual în subcapitolul 2.4. Logica de declanșare este coordonată de serviciul de monitorizare a alertelor, care decide când și prin ce canale trebuie transmisă o notificare, ținând cont de preferințele fiecărui utilizator. Aceste preferințe sunt expuse într-o pagină dedicată de setări, unde fiecare canal (email, push, SMS) și raportul zilnic pot fi activate sau dezactivate independent printr-un comutator.

\subsubsection{Temporizarea alertelor}

Pentru a evita alertele false, serviciul de monitorizare aplică mai multe temporizări. O stare anormală trebuie să persiste o durată minimă, anume pragul de persistență, de aproximativ două minute pentru stările critice, înainte de a genera o notificare, eliminând valorile tranzitorii. La declanșare, alerta este transmisă simultan prin toate canalele activate de utilizator (push, email și SMS); SMS-ul nu este întârziat suplimentar față de celelalte canale. Atâta vreme cât starea critică persistă, notificarea este retrimisă la fiecare două minute; pentru alertele non-critice, un interval de inhibare (\textit{cooldown}) împiedică retransmiterea aceleiași alerte pentru aceeași persoană, eliminând riscul inundării aparținătorului cu mesaje. Pentru aplicarea acestor reguli, serviciul menține în memorie, pentru fiecare persoană monitorizată, un buffer cu măsurătorile din ultimele minute.

\subsubsection{Notificările push}

Sistemul implementează două tipuri complementare de notificări push.

\textbf{Notificările în aplicație (SignalR)} sunt livrate printr-un hub WebSocket la care clienții se conectează după autentificare. Fiecare utilizator este asociat unui grup propriu, astfel încât o notificare să fie direcționată exact către destinatar. Mesajele sunt afișate instantaneu sub forma unui \textit{toast} colorat în funcție de severitate (figura~\ref{fig:notificare-push}) și salvate în baza de date pentru consultare ulterioară.

\textbf{Notificările prin browser (Web Push / VAPID)} completează canalul SignalR cu livrarea prin service worker-ul aplicației, permițând primirea alertelor chiar și atunci când tabul aplicației este închis. La autentificare, aplicația verifică dacă browser-ul suportă Web Push și dacă permisiunea este acordată; dacă da, subscripția este înregistrată pe server împreună cu cheia publică și secretul de autentificare (p256dh/auth). La declanșarea unei alerte, serverul trimite notificarea prin WebPush client (biblioteca \texttt{WebPush}), care livrează payload-ul JSON criptat endpoint-ului browser-ului. Service worker-ul interceptează evenimentul \texttt{push}, afișează notificarea sistemului de operare și, la clic, deschide pagina de notificări a aplicației. Subscripțiile expirate (cod HTTP 410 de la furnizorul browser-ului) sunt eliminate automat din baza de date.

Din pagina de profil, utilizatorul poate activa sau dezactiva independent notificările Web Push prin browser, cu indicarea vizuală a stării curente (activ / dezactivat / blocat de browser).

\begin{figure}[H]
\centering
\includegraphics[width=0.80\textwidth]{../figuri/notificare-push.png}
\caption{Notificare push afișată în interfața web prin SignalR, imediat ce alerta este declanșată pe server.}
\label{fig:notificare-push}
\end{figure}

\subsubsection{SMS și email}

Trimiterea SMS-urilor este realizată prin integrarea cu platforma de comunicație în cloud \textbf{Twilio}, apelată prin biblioteca oficială a acesteia. Mesajul SMS (figura~\ref{fig:notificare-sms}) conține tipul alertei, numele persoanei monitorizate și valorile vitale relevante, format compact pentru a se încadra într-un singur segment. Email-urile sunt trimise printr-un server SMTP, folosind \textit{template}-uri HTML dedicate cu identitatea vizuală a aplicației. Sistemul implementează \textbf{7 tipuri distincte de email}: (1) confirmarea adresei la înregistrare, (2) resetarea parolei, (3) verificarea noii adrese de email la schimbarea acesteia, (4) notificarea adresei vechi la schimbarea email-ului, (5) alertele medicale, (6) invitația medicului și (7) notificarea la adăugarea unei notițe medicale. Fiecare tip are un template separat, bilingv (română/engleză), selectat în funcție de preferința de limbă a destinatarului. Excepție fac emailul de invitație trimis medicului și emailul de confirmare a adresei la înregistrare, care sunt redactate exclusiv \textbf{în limba română}, întrucât destinatarul poate să nu aibă încă un cont cu preferințe configurate. Fiecare utilizator poate activa sau dezactiva independent fiecare canal din pagina de setări, iar sistemul respectă aceste preferințe la livrarea fiecărei alerte.

\begin{figure}[H]
\centering
\includegraphics[width=0.60\textwidth]{../figuri/notificare-sms.png}
\caption{Alertă livrată prin SMS pe telefonul aparținătorului, prin platforma Twilio.}
\label{fig:notificare-sms}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.80\textwidth]{../figuri/notificare-email.png}
\caption{Email de alertă cu \textit{template} HTML dedicat, livrat printr-un server SMTP configurat în aplicație.}
\label{fig:notificare-email}
\end{figure}

\subsubsection{Raportul zilnic}

Pe lângă alertele declanșate de situații de risc, sistemul generează, pentru utilizatorii care au activat această opțiune dintr-un comutator dedicat din pagina de setări, un \textbf{raport zilnic} de sănătate. Serviciul de fundal corespunzător rulează o dată pe zi, la miezul nopții în fusul orar local configurat în setările serverului (de exemplu, \texttt{Europe/Bucharest}), agregă măsurătorile zilei precedente, anume valorile minime, maxime și medii ale parametrilor vitali, numărul de căderi și de alerte, și trimite sinteza prin email folosind un \textbf{template HTML dedicat}, distinct de cel al notificărilor de alertă. Raportul este livrat exclusiv prin email în format HTML; nu se generează un fișier PDF atașat. Acest mecanism oferă aparținătorului o imagine de ansamblu chiar și în zilele fără evenimente, fără a fi necesară consultarea activă a aplicației.

\subsubsection{Feedback pentru alarme false}

Pentru a îmbunătăți calitatea sistemului de alertare în timp și pentru a oferi date concrete despre rata reală de alerte false, aplicația implementează o buclă de feedback: atunci când o stare de alertă se rezolvă (vitalele revin la normal), ultima notificare a episodului este marcată automat în baza de date ca \textit{așteptând feedback}. La următoarea accesare a paginii persoanei monitorizate, un banner vizibil la începutul paginii întreabă explicit dacă alerta a fost reală sau falsă. Răspunsul (\textit{Da, reală}, \textit{Nu, alarmă falsă} sau \textit{Întreabă-mă mai târziu}) este înregistrat pe notificare, iar prima opțiune fixează răspunsul definitiv, în timp ce ultima lasă banner-ul să reapară la accesul ulterior. Datele etichetate astfel permit, ulterior, calcularea ratei reale de alerte false raportate de utilizatori, metrică folosită și în capitolul de testare al lucrării.
...
\begin{itemize}[leftmargin=1.5em]
\item \textbf{Antrenarea pe date clinice reale}: colectarea de date de la pacienți reali, în colaborare cu o instituție medicală, ar permite reantrenarea și validarea clinică a modelului, depășind limitarea actuală a datelor sintetice și îmbunătățind semnificativ acuratețea predicțiilor în condiții reale.

\item \textbf{Retrenare continuă prin bucla de feedback}: sistemul colectează deja etichete reale/false de la utilizatori pentru fiecare alertă. Aceste date pot alimenta periodic un proces de reantrenare a modelului, reducând în timp rata alertelor false specifice unui pacient sau unui profil demografic.

\item \textbf{Aplicație mobilă nativă}: o aplicație dedicată Android/iOS, complementară interfeței web, ar facilita primirea notificărilor push fiabile chiar și fără browser deschis și ar oferi o experiență mai fluentă aparținătorilor care accesează datele în principal de pe telefon.

\item \textbf{Senzori suplimentari}: integrarea unui modul ECG ar permite detectarea aritmiilor; adăugarea unui senzor de tensiune arterială și a unuia de glucoză sanguină ar extinde semnificativ profilul clinic monitorizat, cu beneficii directe pentru pacienții cu hipertensiune și diabet.

\item \textbf{Alerte accesibile pentru persoane cu dizabilități senzoriale}: integrarea unor modalități alternative de alertare — mesaje audio pentru utilizatorii cu deficiențe de vedere și semnale tactile (vibrații) pentru cei cu deficiențe de auz — ar extinde accesibilitatea sistemului la un public mai larg din rândul vârstnicilor.

\item \textbf{Integrare cu serviciile de urgență}: o funcționalitate de alertare automată a serviciului 112 în situații critice confirmate, cu transmiterea coordonatelor GPS ale pacientului, ar reduce intervalul de răspuns în cazul căderilor sau al deteriorărilor rapide ale stării de sănătate.

\item \textbf{Integrare cu sistemele EHR}: conectarea la sisteme informatice medicale standardizate (HL7 FHIR) ar permite medicului curent să consulte datele de monitorizare direct din fișa electronică a pacientului, fără a fi necesar accesul separat la aplicație.

\item \textbf{Analiză longitudinală a stării de sănătate}: extinderea predicției de tendințe de la orizontul de minute (descris în subcapitolul 4.4) la o analiză pe zile și săptămâni ar permite detectarea degradărilor lente ale stării de sănătate, invizibile la nivelul unui singur episod, și corelarea acestora cu factorii contextuali precum activitatea fizică sau temperatura ambiantă.

\item \textbf{Miniaturizare și certificare medicală}: prototipul actual demonstrează fezabilitatea conceptului; un produs comercializabil ar presupune redesignul hardware pe o placă PCB dedicată, miniaturizarea carcasei, optimizarea consumului pentru autonomie de câteva zile pe o singură încărcare și obținerea certificărilor medicale necesare comercializării.
\end{itemize}
...
\bibitem{alshamrani2022} M. Alshamrani, ``IoT and artificial intelligence implementations for remote healthcare monitoring systems: A survey,'' \textit{Journal of King Saud University -- Computer and Information Sciences}, vol. 34, nr. 8, pp. 4687--4701, 2022. \url{https://doi.org/10.1016/j.jksuci.2021.06.005}.

\bibitem{nancy2022} A. A. Nancy, D. Ravindran, P. M. D. R. Vincent, K. Srinivasan, D. G. Reina, ``IoT-Cloud-Based Smart Healthcare Monitoring System for Heart Disease Prediction via Deep Learning,'' \textit{Electronics}, vol. 11, nr. 15, art. 2292, 2022. \url{https://doi.org/10.3390/electronics11152292}.

\bibitem{shah2026} T. Shah, K. Dumasia, H. J. Lad, K. Shah, K. Nadiyadra, V. Suratwala, ``A systematic review of emerging trends of IoT in healthcare and IoMT frameworks,'' \textit{Discover Public Health}, vol. 23, nr. 1, art. 52, 2026. \url{https://doi.org/10.1186/s12982-026-01347-w}.

\bibitem{bansal2026} J. C. Bansal, S. Borah, S. Hussain, S. Salhi (eds.), \textit{Computing and Machine Learning: Proceedings of CML 2025, vol. 1}. Springer Nature, 2026, ISBN 978-981-952-872-1.

\bibitem{maswadi2026} K. Maswadi, A. Alhazmi, ``Towards Sustainable Health Management: The Role of Artificial Intelligence -- A Systematic Review, Challenges, and Future Directions,'' \textit{Sustainability}, vol. 18, nr. 2, art. 905, 2026. \url{https://doi.org/10.3390/su18020905}.

\bibitem{pabitha2025} C. Pabitha, S. Benila, G. Sangeetha, A. Vidhya, ``IoT and Cloud-Enabled AI Predictive Maintenance for Manufacturing, Energy, Healthcare Systems,'' în \textit{Advanced Materials for Biomedical Devices}. CRC Press, 2025. \url{https://doi.org/10.1201/9781003651772-38}.

\bibitem{mollabagher2026} M. Mollabagher, A. Hassanzadeh, M. M. Sepehri, A. Habibelahi, A. Sarabadani, ``Towards an Integrated Framework for Health Surveillance Systems: A Systematic Literature Review of Design Components and Implementation Challenges,'' \textit{Health Science Reports}, vol. 9, nr. 1, art. e71652, 2026. \url{https://doi.org/10.1002/hsr2.71652}.

\bibitem{bandaru2025} V. K. R. Bandaru, S. R. Bandaru, ``The Rise of Sustainable IoT in Modern Healthcare Through Trends, Technologies, and Future Directions,'' în \textit{Development and Management of Eco-Conscious IoT Medical Devices}. IGI Global, 2025, pp. 1--28. \url{https://doi.org/10.4018/979-8-3373-4134-7.ch001}.

\bibitem{panahi2025ejiss} O. Panahi, ``Secure IoT for Healthcare,'' \textit{European Journal of Innovative Studies and Sustainability}, vol. 1, nr. 1, pp. 17--23, 2025. \url{https://doi.org/10.59324/ejiss.2025.1(1).03}.

\bibitem{ali2025} T. E. Ali, F. I. Ali, P. Dakić, A. D. Zoltan, ``Trends, prospects, challenges, and security in the healthcare internet of things,'' \textit{Computing}, vol. 107, nr. 1, art. 28, 2025. \url{https://doi.org/10.1007/s00607-024-01352-4}.

\bibitem{panahi2025future} O. Panahi, ``The Future of Healthcare: AI, Public Health and the Digital Revolution,'' \textit{Medical \& Clinical Case Reports Journal}, vol. 3, nr. 1, pp. 763--766, 2025. \url{https://doi.org/10.51219/mccrj/omid-panahi/198}.

\bibitem{panteli2025} D. Panteli et al., ``Artificial intelligence in public health: promises, challenges, and an agenda for policy makers and public health institutions,'' \textit{The Lancet Public Health}, vol. 10, nr. 5, pp. e428--e432, 2025. \url{https://doi.org/10.1016/S2468-2667(25)00036-2}.

\bibitem{ahmadzadeh2024} M. Ahmadzadeh, S. M. Zahrai, M. Bitaraf, ``An integrated deep neural network model combining 1D CNN and LSTM for structural health monitoring utilizing multisensor time-series data,'' \textit{Structural Health Monitoring}, vol. 24, nr. 1, pp. 447--465, 2025. \url{https://doi.org/10.1177/14759217241239041}.
