...
\begin{itemize}[leftmargin=1.5em]
\item \textbf{Limitarea ratei} (\textit{rate limiting}) restricționează numărul de cereri de autentificare de la același client, atenuând atacurile de tip forță brută;
...
Trimiterea SMS-urilor este realizată prin integrarea cu platforma de comunicație în cloud \textbf{Twilio}, apelată prin biblioteca oficială a acesteia. Mesajul SMS (figura~\ref{fig:notificare-sms}) conține tipul alertei, numele persoanei monitorizate și valorile vitale relevante, format compact pentru a se încadra într-un singur segment. Email-urile (figura~\ref{fig:notificare-email}), atât notificările, cât și mesajele tranzacționale precum confirmarea contului sau resetarea parolei, sunt trimise printr-un server SMTP, folosind un \textit{template} HTML cu identitatea vizuală a aplicației. Fiecare utilizator poate activa sau dezactiva independent fiecare canal din pagina de setări, iar sistemul respectă aceste preferințe la livrarea fiecărei alerte.
